        ...
        <div id="game-phase-panel" class="window-body"></div>
            ...
            <div class="flex gap-2">
                <div class="flex-grow space-y-2">
                    <div data-tooltip-key="adventurer_health">
                        <div class="flex justify-between items-center">
                            <div class="flex items-center">${qe()} <span>${r("global.health")}</span></div>
                ...
                <div class="sunken-panel p-2 flex flex-col items-center justify-center" data-tooltip-key="adventurer_power">
                    <div class="flex items-center">${Be()} <span class="ml-1">${r("global.power")}</span></div>
    ...
    `;const e=this.querySelector("#item-cards");if(e){e.innerHTML="";for(const t of this._items){const n=document.createElement("choice-card");n.item=t,n.purchaseInfo={cost:t.cost||0,canAfford:this._balancePoints>=(t.cost||0)},e.appendChild(n)}}}}customElements.define("workshop-screen",Qe);class et extends HTMLElement{constructor(){super(),this.engine=null,this.metaManager=null,this.addEventListener("click",async e=>{if(!this.engine||!this.metaManager)return;const t=e.target;t.id==="new-game-button"?(this.metaManager.metaState.adventurers>0||this.metaManager.metaState.highestRun>0,this.engine.hasSaveGame()?await j.show(r("menu.new_game"),r("menu.new_game_confirm"))&&this.engine.startNewGame():this.engine.startNewGame()):t.id==="continue-game-button"?this.engine.continueGame():t.id==="reset-game-button"&&await j.show(r("menu.reset_save"),r("menu.reset_save_confirm"))&&(this.metaManager.reset(),this.engine.quitGame(!0),this.render())})}connectedCallback(){this.render()}render(){if(!this.metaManager||!this.engine)return;const e=this.metaManager.metaState,t=this.engine.hasSaveGame();let n="";if(t){const s=e.adventurers||0;n=`
      ...
      <div class="window" style="width: 400px;">
        ...
        <div class="status-bar">
          ...
          <p class="status-bar-field">build 189</p>
        </div>
      </div>
...
.connector-label {
  ...
  user-select: none;
    ...
    `,this.shadowRoot.appendChild(e)}_createSVGElement(e,t={}){const n=document.createElementNS("http://www.w3.org/2000/svg",e);for(const s in t)n.setAttribute(s,t[s]);return n}_prepareTree(e,t=0,n=null){const s=e,a=n;if(s.depth=t,s.parent=a,this._maxDepth=Math.max(this._maxDepth,t),this._allNodes.push(s),!s.children||s.children.length===0)return s.leafCount=1,1;let i=0;for(const l of s.children)i+=this._prepareTree(l,t+1,s);return s.leafCount=i,i}_calculatePositions(e,t,n){const a=this._maxDepth*(O+U)-e.depth*(O+U),i=t+(n-t)/2-T/2;if(this._positions.set(e.id,{x:i,y:a}),!e.children||e.children.length===0)return;let l=t;for(const c of e.children){const d=(n-t)*c.leafCount/e.leafCount;this._calculatePositions(c,l,l+d),l+=d}}_drawConnector(e,t){const n=this._positions.get(e.id),s=this._positions.get(t.id),a=this._data.path.includes(t.id)&&this._data.path.includes(e.id),i=n.x+T/2,l=n.y,c=s.x+T/2,d=s.y+O,u=l-U/2,h=this._createSVGElement("g"),m=this._createSVGElement("path",{d:`M ${i} ${l} V ${u} H ${c} V ${d}`,stroke:a?"var(--path-connector-color)":"var(--connector-color)","stroke-width":a?"2.5":"1.5",fill:"none"});h.appendChild(m);const f=this._data.linkLabels[t.id];if(f){const g=this._createSVGElement("text",{class:"connector-label",x:c+"",y:u+"",dy:"-4px"});g.textContent=f,a&&(g.style.fill="var(--path-connector-color)",g.style.fontWeight="700"),h.appendChild(g)}a?this._groupEl.appendChild(h):this._groupEl.prepend(h)}_drawNode(e){const t=this._positions.get(e.id),n=this._data.path.includes(e.id),s=this._createSVGElement("g"),a=this._createSVGElement("rect",{x:t.x,y:t.y,width:T+"",height:O+"",fill:n?"var(--path-node-bg)":"var(--node-bg)",stroke:n?"var(--path-node-border)":"var(--node-border)",class:"node-rect",filter:"url(#shadow)"});s.appendChild(a);const i=this._createSVGElement("text",{class:"node-text"}),l=e.label.split(" ");let c="";const d=[],u=T-20;for(let f=0;f<l.length;f++){const g=c+l[f]+" ",p=this._createSVGElement("text",{class:"node-text",style:"visibility: hidden;"});p.textContent=g,this._svgEl.appendChild(p);const y=p.getComputedTextLength();this._svgEl.removeChild(p),y>u&&f>0?(d.push(c),c=l[f]+" "):c=g}d.push(c);const h=16,m=t.y+O/2-(d.length-1)*h/2;i.setAttribute("y",m+""),d.forEach((f,g)=>{const p=this._createSVGElement("tspan",{x:t.x+T/2,dy:g===0?"0":`${h}px`});p.textContent=f.trim(),i.appendChild(p)}),s.appendChild(i),this._groupEl.appendChild(s)}_createDefs(){const e=this._createSVGElement("defs"),t=this._createSVGElement("filter",{id:"shadow",x:"-50%",y:"-50%",width:"200%",height:"200%"}),n=this._createSVGElement("feDropShadow",{dx:"2",dy:"2",stdDeviation:"3","flood-color":"var(--node-shadow)"});t.appendChild(n),e.appendChild(t),this._groupEl.parentElement.appendChild(e)}_getTouchDistance(e){const[t,n]=[e[0],e[1]];return Math.sqrt(Math.pow(n.clientX-t.clientX,2)+Math.pow(n.clientY-t.clientY,2))}_setupZoomAndPan(){let e=!1,t={x:0,y:0},n=0;const s=()=>{this._groupEl.setAttribute("transform",`translate(${this._transform.tx}, ${this._transform.ty}) scale(${this._transform.scale})`)};this._svgEl.addEventListener("wheel",i=>{i.preventDefault();const l=1.1,c=i.deltaY<0?l:1/l,d=this._svgEl.getBoundingClientRect(),u=i.clientX-d.left,h=i.clientY-d.top;this._transform.tx=u-(u-this._transform.tx)*c,this._transform.ty=h-(h-this._transform.ty)*c,this._transform.scale*=c,s()}),this._svgEl.addEventListener("mousedown",i=>{e=!0,t={x:i.clientX,y:i.clientY},this._svgEl.style.cursor="grabbing"}),this._svgEl.addEventListener("mousemove",i=>{if(!e)return;const l={x:i.clientX,y:i.clientY},c=l.x-t.x,d=l.y-t.y;this._transform.tx+=c,this._transform.ty+=d,t=l,s()});const a=()=>{e&&(e=!1,this._svgEl.style.cursor="grab")};this._svgEl.addEventListener("mouseup",a),this._svgEl.addEventListener("mouseleave",a),this._svgEl.addEventListener("touchstart",i=>{i.touches.length===1?(e=!0,t={x:i.touches[0].clientX,y:i.touches[0].clientY},this._svgEl.style.cursor="grabbing"):i.touches.length===2&&(n=this._getTouchDistance(i.touches))}),this._svgEl.addEventListener("touchmove",i=>{if(i.touches.length===1&&e){i.preventDefault();const l={x:i.touches[0].clientX,y:i.touches[0].clientY},c=l.x-t.x,d=l.y-t.y;this._transform.tx+=c,this._transform.ty+=d,t=l,s()}else if(i.touches.length===2){i.preventDefault();const l=this._getTouchDistance(i.touches),c=l/n,d=this._svgEl.getBoundingClientRect(),u=i.touches[0],h=i.touches[1],m=(u.clientX+h.clientX)/2-d.left,f=(u.clientY+h.clientY)/2-d.top;this._transform.tx=m-(m-this._transform.tx)*c,this._transform.ty=f-(f-this._transform.ty)*c,this._transform.scale*=c,n=l,s()}}),this._svgEl.addEventListener("touchend",i=>{i.touches.length<2&&a()})}_centerChart(){const e=this._groupEl.getBBox(),t=this.getBoundingClientRect();if(e.width===0||e.height===0)return;const n=80,s=t.width/(e.width+n),a=t.height/(e.height+n),i=Math.min(s,a,1);this._transform.scale=i,this._transform.tx=t.width/2-(e.x+e.width/2)*i,this._transform.ty=t.height/2-(e.y+e.height/2)*i,this._groupEl.setAttribute("transform",`translate(${this._transform.tx}, ${this._transform.ty}) scale(${this._transform.scale})`)}render(){if(this._svgEl&&this.shadowRoot.removeChild(this._svgEl),!this._data)return;this._svgEl=this._createSVGElement("svg"),this.shadowRoot.appendChild(this._svgEl),this._groupEl=this._createSVGElement("g"),this._svgEl.appendChild(this._groupEl),this._createDefs(),this._prepareTree(this._data.nodes);const e=this._data.nodes.leafCount*(T+it);this._calculatePositions(this._data.nodes,0,e);for(const t of this._allNodes)t.parent&&this._drawConnector(t.parent,t);for(const t of this._allNodes)this._drawNode(t);this._centerChart(),this._setupZoomAndPan()}}customElements.define("dungeon-chart",at);const $=document.getElementById("app");if(!$)throw new Error("Could not find app element to mount to");async function ot(){$.innerHTML="<div>Initializing...</div>";const o=new Pe;await ue(o);const e=new Oe,t=new Me(e),n=new Ae(e),s=new $e(t,o,n);s.on("state-change",a=>{if(s.isLoading){$.innerHTML=`<div>${r("global.loading_game_data")}</div>`;return}if(s.error){$.innerHTML=`
            ...
            `;return}Te($,a,s)}),s.on("show-encounter",async a=>{const{EncounterModal:i}=await de(async()=>{const{EncounterModal:l}=await Promise.resolve().then(()=>nt);return{EncounterModal:l}},void 0);await i.show(a),s.continueEncounter()}),$.innerHTML=`<div>${r("global.initializing")}</div>`,document.body.addEventListener("mouseover",a=>q.handleMouseEnter(a)),document.body.addEventListener("click",a=>q.handleClick(a)),await s.init(),s.showMenu()}ot().catch(o=>{console.error(o),$&&($.innerHTML=`
      <div class="min-h-screen flex items-center justify-center p-4">
          <div class="bg-brand-surface p-8 rounded-xl shadow-2xl text-center border border-brand-secondary">
