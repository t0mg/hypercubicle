(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))t(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function s(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function t(r){if(r.ep)return;r.ep=!0;const a=s(r);fetch(r.href,a)}})();const H=15,N=3,A={Common:1,Uncommon:2,Rare:3},E={hp:100,maxHp:100,power:5},P=10,I=10,q=["loot_1","loot_2","loot_3","loot_4","loot_5"],R=32,w=9,C=300,O=n=>[...n].sort(()=>Math.random()-.5),M=(n,e)=>{let s=0;const t=new Map(e.map(o=>[o.id,o])),r=n.map(o=>t.get(o)).filter(Boolean),a=o=>(s++,{...o,instanceId:`${o.id}-${s}`}),i=r.filter(o=>o.cost===null),p=r.filter(o=>o.cost!==null);let m=[];if(i.length>0)for(const o of i)m.push(a(o),a(o),a(o),a(o));m.push(...p.map(a));let c=0;for(;m.length<R&&i.length>0;)m.push(a(i[c%i.length])),c++;return O(m)};class B{constructor(){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._listeners={},this.startNewGame=()=>{const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},s={...E,interest:33+Math.floor(Math.random()*50),traits:e,inventory:{weapon:null,armor:null,potions:[]}},t=q,r=M(t,this._allItems),a=r.slice(0,w),i=r.slice(w);this.gameState&&this.gameState.log&&console.log("previous game log dump:",this.gameState.log),this.gameState={phase:"DESIGNER_CHOOSING_DIFFICULTY",designer:{balancePoints:0},adventurer:s,unlockedDeck:t,availableDeck:i,hand:a,shopItems:[],offeredLoot:[],feedback:"A new adventurer enters the dungeon!",log:["--- Starting New Game (Run 1) ---"],run:1,floor:1,gameOver:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.startNewRun=()=>{if(!this.gameState)return;const e=M(this.gameState.unlockedDeck,this._allItems),s=e.slice(0,w),t=e.slice(w),r={...E,interest:this.gameState.adventurer.interest,traits:this.gameState.adventurer.traits,inventory:{weapon:null,armor:null,potions:[]}};this.gameState={...this.gameState,adventurer:r,phase:"DESIGNER_CHOOSING_DIFFICULTY",availableDeck:t,hand:s,floor:1,feedback:"The adventurer returns for another run!",log:[...this.gameState.log,`--- Starting Run ${this.gameState.run} ---`],gameOver:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=e=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(t=>e.includes(t.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;const{choice:t,reason:r,logs:a}=this._getAdventurerChoice(this.gameState.adventurer,this.gameState.offeredLoot);let i={...this.gameState.adventurer},p=i.interest,m=this.gameState.hand,c=this.gameState.availableDeck;m.forEach(h=>h.justDrafted=!1);let o=m.filter(h=>!e.includes(h.instanceId));const f=w-o.length,v=c.slice(0,f);v.forEach(h=>{h.draftedFloor=this.gameState.floor,h.justDrafted=!0});const b=c.slice(f);if(o.push(...v),t){let h={...i.inventory,potions:[...i.inventory.potions]};t.type==="Weapon"?h.weapon=t:t.type==="Armor"?h.armor=t:t.type==="Potion"&&h.potions.push(t);const{power:_,maxHp:x}=this._recalculateStats(h),y=x-i.maxHp;i.inventory=h,i.power=_,i.maxHp=x,i.hp+=Math.max(0,y)}else p=Math.max(0,this.gameState.adventurer.interest-10);const d=this.gameState.floor+1,l=this.gameState.designer.balancePoints+I;this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:{...i,interest:p},feedback:r,availableDeck:b,hand:o,log:[...this.gameState.log,...a],floor:d,designer:{balancePoints:l}},this._emit("state-change",this.gameState)},C)},this.runDebugEncounter=e=>{!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_DIFFICULTY"||(this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.debugEncounterParams=e,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.debugEncounterParams)return;const{newAdventurer:s,feedback:t,logs:r}=this._getDebugEncounterOutcome(this.gameState.adventurer,this.gameState.floor,this.gameState.debugEncounterParams),a=[...this.gameState.log,...r];if(s.hp<=0){a.push("GAME OVER: Adventurer has fallen in battle."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:this.gameState.designer.balancePoints+I},phase:"RUN_OVER",gameOver:{isOver:!0,reason:`The adventurer fell on floor ${this.gameState.floor} of run ${this.gameState.run}.`},log:a},this._emit("state-change",this.gameState);return}if(s.interest<=H){a.push("GAME OVER: Adventurer lost interest and quit."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:this.gameState.designer.balancePoints+I},phase:"RUN_OVER",gameOver:{isOver:!0,reason:`The adventurer grew bored on floor ${this.gameState.floor} of run ${this.gameState.run}, and quit.`},log:a},this._emit("state-change",this.gameState);return}let i=t;this.gameState.hand&&this.gameState.hand.length===0?(a.push("Your hand is empty! The adventurer must press on without new items."),i="The adventurer waits for your decision, unaware that you have nothing left to offer.",this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:s,floor:this.gameState.floor+1,designer:{balancePoints:this.gameState.designer.balancePoints+I},feedback:i,log:a,debugEncounterParams:void 0}):this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",adventurer:s,feedback:i,log:a,debugEncounterParams:void 0},this._emit("state-change",this.gameState)},C))},this.enterWorkshop=()=>{if(!this.gameState)return;const e=this.gameState.run+1,s=this._allItems.filter(t=>t.cost!==null).filter(t=>!this.gameState.unlockedDeck.includes(t.id)).filter(t=>e>=t.minRun);this.gameState={...this.gameState,phase:"SHOP",run:e,floor:0,shopItems:O(s).slice(0,4),gameOver:{isOver:!1,reason:""},feedback:"Welcome back. Spend your Balance Points wisely."},this._emit("state-change",this.gameState)},this.purchaseItem=e=>{if(!this.gameState)return;const s=this._allItems.find(i=>i.id===e);if(!s||s.cost===null||this.gameState.designer.balancePoints<s.cost)return;const t=[...this.gameState.unlockedDeck,e],r=this.gameState.designer.balancePoints-s.cost,a=this.gameState.shopItems.filter(i=>i.id!==e);this.gameState={...this.gameState,designer:{balancePoints:r},unlockedDeck:t,shopItems:a,log:[...this.gameState.log,`Purchased ${s.name}.`]},this._emit("state-change",this.gameState)},this._loadGameData()}on(e,s){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(s)}_emit(e,s){const t=this._listeners[e];t&&t.forEach(r=>r(s))}_recalculateStats(e){let s=E.power,t=E.maxHp;return e.weapon&&(s+=e.weapon.stats.power||0,t+=e.weapon.stats.maxHp||0),e.armor&&(s+=e.armor.stats.power||0,t+=e.armor.stats.maxHp||0),{power:s,maxHp:t}}_getAdventurerChoice(e,s){var v,b;const{traits:t,inventory:r}=e,a=[`--- Adventurer Decision --- (Offense: ${t.offense}, Risk: ${t.risk})`],i=((v=r.weapon)==null?void 0:v.stats.power)||0,p=((b=r.armor)==null?void 0:b.stats.maxHp)||0;a.push(`Current Gear: Weapon Power(${i}), Armor HP(${p})`);const m=d=>{var h,_;let l=(A[d.rarity]||1)*5;switch(d.type){case"Weapon":const x=(d.stats.power||0)-i;if(x<=0&&d.id!==((h=r.weapon)==null?void 0:h.id))return-1;l+=x*(t.offense/10),x>0&&(l+=x*(t.expertise/10));const y=d.stats.maxHp||0;y<0&&(l+=y*(100-t.risk)/20);break;case"Armor":const S=(d.stats.maxHp||0)-p;if(S<=0&&d.id!==((_=r.armor)==null?void 0:_.id))return-1;l+=S*(100-t.offense)/10,S>0&&(l+=S*(t.expertise/10));const $=d.stats.power||0;$>0&&(l+=$*(t.offense/15));const L=d.stats.power||0;L<0&&(l+=L*(t.risk/10));break;case"Potion":const k=e.hp/e.maxHp;l+=10*(100-t.risk)/100,k<.7&&(l+=20*(1-k)),l+=5*(t.expertise/100),r.potions.length>=N&&(l*=.1);break}return l+Math.random()},c=s.map(d=>({item:d,score:m(d)})).filter(d=>d.score>0);if(c.sort((d,l)=>l.score-d.score),c.length===0||c[0].score<P)return{choice:null,reason:"They examine your offers but decide to take nothing.",logs:a};const o=c[0].item;let f=`They pick the ${o.name}.`;return{choice:o,reason:f,logs:a}}_getDebugEncounterOutcome(e,s,t){const r=[`--- DEBUG Encounter: Floor ${s} ---`];let a=JSON.parse(JSON.stringify(e)),i="";const p=a.hp/a.maxHp,m=1-a.traits.risk/120;if(p<m&&a.inventory.potions.length>0){const d=a.inventory.potions.shift(),l=d.stats.hp||0;a.hp=Math.min(a.maxHp,a.hp+l),i=`Sensing danger, they drink a ${d.name}. `}let c=Math.round(t.baseDamage*t.difficultyFactor);c=Math.max(0,c),a.hp-=c;let o=0,f="";const v=c/a.maxHp;let b="Normal";return t.difficultyFactor<=.8?b="Easy":t.difficultyFactor>=1.3&&(b="Hard"),b==="Easy"?(o=-5-Math.floor((100-a.traits.risk)/20),f="An easy fight. Too easy..."):v<.1?(o=-2,f="A worthy, but simple challenge."):v<.35?(o=5+Math.floor(a.traits.offense/20)+(b==="Hard"?5:0),f="A great battle! They feel alive!"):(o=-10-Math.floor((100-a.traits.risk)/10),f="That was far too close for comfort."),a.interest=Math.max(0,Math.min(100,a.interest+o)),a.traits.expertise+=1,{newAdventurer:a,feedback:i+f,damageTaken:c,logs:r}}getAdventurerEndRunDecision(){if(!this.gameState)return"retire";const{interest:e}=this.gameState.adventurer,s=e-H,t=(Math.random()-.5)*20;return s+t>0?"continue":"retire"}async _loadGameData(){try{const e=await fetch("/game/items.json");if(!e.ok)throw new Error(`Failed to load items.json: ${e.statusText}`);this._allItems=await e.json(),this.startNewGame()}catch(e){this.error=e.message||"An unknown error occurred while loading game data.",this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}class G extends HTMLElement{constructor(){super(),this._balancePoints=0,this._run=0,this._floor=0}static get observedAttributes(){return["balance-points","run","floor"]}attributeChangedCallback(e,s,t){switch(e){case"balance-points":this._balancePoints=Number(t);break;case"run":this._run=Number(t);break;case"floor":this._floor=Number(t);break}this.render()}connectedCallback(){this.render()}render(){this.innerHTML=`
                    ...
                    <div class="space-y-4">
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${U()} <span class="font-semibold text-lg">Health</span></div>
    ...
    `};class z extends HTMLElement{constructor(){super(),this._item=null,this._isSelected=!1,this._isDisabled=!1,this._isNewlyDrafted=!1,this.addEventListener("click",()=>{!this._isDisabled&&this._item&&this.dispatchEvent(new CustomEvent("loot-card-select",{bubbles:!0,composed:!0,detail:{instanceId:this._item.instanceId}}))}),this.addEventListener("animationend",e=>{e.animationName==="newly-drafted-animation"&&this.classList.remove("animate-newly-drafted")})}set item(e){this._item=e,this.render()}get item(){return this._item}set isSelected(e){this._isSelected=e,this.render()}get isSelected(){return this._isSelected}set isDisabled(e){this._isDisabled=e,this.render()}get isDisabled(){return this._isDisabled}set isNewlyDrafted(e){this._isNewlyDrafted!==e&&(this._isNewlyDrafted=e,this._isNewlyDrafted&&this.classList.add("animate-newly-drafted"))}get isNewlyDrafted(){return this._isNewlyDrafted}connectedCallback(){this.render()}render(){if(!this._item)return;const e=K[this._item.rarity]||"text-gray-400",s="bg-brand-surface border rounded-lg p-4 flex flex-col justify-between transition-all duration-200 shadow-lg";let t="";this._isDisabled?t="border-gray-600 opacity-50 cursor-not-allowed":this._isSelected?t="border-brand-secondary scale-105 ring-2 ring-brand-secondary cursor-pointer transform":t="border-brand-primary hover:border-brand-secondary cursor-pointer transform hover:scale-105";const r=this.classList.contains("animate-newly-drafted")?" animate-newly-drafted":"";this.className=`${s} ${t}${r}`,this.innerHTML=`
            <div>
                ...
                <div class="space-y-1 text-brand-text">
                    ${this._item.stats.hp?T("Health",this._item.stats.hp):""}
                    ${this._item.stats.maxHp?T("Max HP",this._item.stats.maxHp):""}
                    ${this._item.stats.power?T("Power",this._item.stats.power):""}
                </div>
            </div>
        ...
        `}}customElements.define("loading-indicator",X);class ee extends HTMLElement{constructor(){super(),this._defaultBaseDamage=10,this.addEventListener("submit",e=>{e.preventDefault();const s=this.querySelector("#base-damage"),t=this.querySelector("#difficulty-factor"),r={baseDamage:parseFloat(s.value)||0,difficultyFactor:parseFloat(t.value)||0};this.dispatchEvent(new CustomEvent("run-encounter",{bubbles:!0,composed:!0,detail:{params:r}}))})}set defaultBaseDamage(e){this._defaultBaseDamage=e;const s=this.querySelector("#base-damage");s&&(s.value=this._defaultBaseDamage.toString())}connectedCallback(){this.render(),this.defaultBaseDamage=this._defaultBaseDamage}render(){this.innerHTML=`
        ...
        `}}customElements.define("debug-encounter-panel",ee);class te extends HTMLElement{constructor(){super(),this._logs=[],this._traits=null}set logs(e){this._logs=e,this.render()}set traits(e){this._traits=e,this.render()}connectedCallback(){this.render()}render(){if(!this._traits){this.innerHTML="";return}const e=this._logs.map((t,r)=>`<p class="whitespace-pre-wrap">[${r.toString().padStart(3,"0")}] ${t}</p>`).join("");this.innerHTML=`
        ...
        `;const s=this.querySelector("#log-container");s&&(s.scrollTop=s.scrollHeight)}}customElements.define("debug-log",te);class se extends HTMLElement{constructor(){super(),this.state="initial",this.decision=null,this.addEventListener("click",e=>{const s=e.target;s.id==="enter-workshop-button"?this.dispatchEvent(new CustomEvent("enter-workshop",{bubbles:!0,composed:!0})):s.id==="new-adventurer-button"&&this.dispatchEvent(new CustomEvent("start-game",{bubbles:!0,composed:!0}))})}connectedCallback(){this.state==="initial"&&(this.render(),this.revealDecision())}revealDecision(){this.state="revealing",setTimeout(()=>{this.decision=this.getAttribute("decision")||"retire",this.state="revealed",this.updateDecision(!0)},2e3)}render(){const e=this.getAttribute("final-bp")||0,s=this.getAttribute("reason")||"The run has ended.";this.innerHTML=`
            <style>
        ...
        `}updateDecision(e){const s=this.querySelector("#decision-container"),t=this.querySelector("#button-container");if(!s||!t)return;let r="",a="";const i=e?"animate-fade-in-up":"";this.decision==="continue"?(r=`
                <h3 class="text-2xl font-bold text-green-400 mb-2 ${i}">"I will go on."</h3>
                <p class="text-brand-text mb-4 ${i}" style="animation-delay: 0.5s;">The adventurer decides to press on, ready for another run!</p>
            `,a+=`
                <button
                    id="enter-workshop-button"
                    class="bg-green-500 text-white font-bold py-3 px-6 rounded-lg hover:bg-green-400 transition-colors transform hover:scale-105 ${i}"
                    style="animation-delay: 1.2s;"
                >
                    Enter Workshop
                </button>
            `):(r=`
                <h3 class="text-2xl font-bold text-red-400 mb-2 ${i}">"I have seen enough."</h3>
                <p class="text-brand-text mb-4 ${i}" style="animation-delay: 0.5s;">The adventurer has lost motivation, and decides to retire.</p>
            `,a+=`
                <button
                    id="new-adventurer-button"
                    class="bg-brand-secondary text-white font-bold py-3 px-6 rounded-lg hover:bg-red-500 transition-colors transform hover:scale-105 ${i}"
                    style="animation-delay: 1s;"
                >
                    Recruit New Adventurer
                </button>
            `),s.innerHTML=r,t.innerHTML=a}}customElements.define("game-over-screen",se);const D=(n,e)=>{const s=e>0;return`
        <div class="flex justify-between text-sm ${s?"text-green-400":"text-red-400"}">
            <span>${n}</span>
            <span class="font-mono">${s?"+":""}${e}</span>
        </div>
    `},ae=(n,e)=>{const t={Common:"text-rarity-common",Uncommon:"text-rarity-uncommon",Rare:"text-rarity-rare"}[n.rarity]||"text-gray-400";return`
        <div class="bg-brand-surface border border-brand-primary rounded-lg p-4 flex flex-col justify-between shadow-lg animate-fade-in">
            <div>
                ...
                <div class="space-y-1 text-brand-text mb-4">
                    ${n.stats.hp?D("Health",n.stats.hp):""}
                    ${n.stats.maxHp?D("Max HP",n.stats.maxHp):""}
                    ${n.stats.power?D("Power",n.stats.power):""}
                </div>
            </div>
    ...
    `};class ne extends HTMLElement{constructor(){super(),this._items=[],this._balancePoints=0,this.addEventListener("click",e=>{const s=e.target,t=s.dataset.itemId;t&&this.dispatchEvent(new CustomEvent("purchase-item",{bubbles:!0,composed:!0,detail:{itemId:t}})),s.id==="start-run-button"&&this.dispatchEvent(new CustomEvent("start-run",{bubbles:!0,composed:!0}))})}set items(e){this._items=e,this.render()}set balancePoints(e){this._balancePoints=e,this.render()}connectedCallback(){this.render()}render(){const e=this._items.map(s=>ae(s,this._balancePoints>=(s.cost||0))).join("");this.innerHTML=`
        ...
        `}}customElements.define("workshop-screen",ne);const u=document.getElementById("app");if(!u)throw new Error("Could not find app element to mount to");const g=new B,re=n=>{if(!n)return"Initializing...";switch(n.phase){case"AWAITING_ADVENTURER_CHOICE":return"Adventurer is considering your offer...";case"AWAITING_ENCOUNTER_FEEDBACK":return"Adventurer is facing the encounter...";default:return"Loading..."}},ie=n=>{switch(n.phase){case"DESIGNER_CHOOSING_LOOT":return'<div class="lg:col-span-3"><loot-choice-panel></loot-choice-panel></div>';case"DESIGNER_CHOOSING_DIFFICULTY":return'<div class="lg:col-span-3"><debug-encounter-panel></debug-encounter-panel></div>';case"AWAITING_ADVENTURER_CHOICE":case"AWAITING_ENCOUNTER_FEEDBACK":return`<div class="lg:col-span-3"><loading-indicator text="${re(n)}"></loading-indicator></div>`;default:return`<div class="lg:col-span-3"><div>Unhandled game phase: ${n.phase}</div></div>`}},oe=n=>{if(!n){u.innerHTML="<div>Loading...</div>";return}if(n.phase==="SHOP"){u.innerHTML="<workshop-screen></workshop-screen>";const i=document.querySelector("workshop-screen");i&&(i.items=n.shopItems,i.balancePoints=n.designer.balancePoints);return}const e=n.gameOver.isOver?`<game-over-screen
                final-bp="${n.designer.balancePoints}"
                reason="${n.gameOver.reason}"
                run="${n.run}"
                decision="${g.getAdventurerEndRunDecision()}"
            ></game-over-screen>`:"";u.innerHTML=`
    ...
    `;const s=document.querySelector("adventurer-status");s&&(s.adventurer=n.adventurer);const t=document.querySelector("loot-choice-panel");t&&(t.choices=n.hand,t.disabled=!1);const r=document.querySelector("debug-encounter-panel");if(r){const i=Math.max(1,15-Math.floor(n.adventurer.power/4)+Math.floor(n.floor*1.5));r.defaultBaseDamage=i}const a=document.querySelector("debug-log");a&&(a.logs=n.log,a.traits=n.adventurer.traits)};u.addEventListener("present-offer",n=>{const{ids:e}=n.detail;g.presentOffer(e)});u.addEventListener("run-encounter",n=>{const{params:e}=n.detail;g.runDebugEncounter(e)});u.addEventListener("enter-workshop",()=>{g.enterWorkshop()});u.addEventListener("purchase-item",n=>{const{itemId:e}=n.detail;g.purchaseItem(e)});u.addEventListener("start-run",()=>{g.startNewRun()});u.addEventListener("start-game",()=>{g.startNewGame()});g.on("state-change",n=>{if(g.isLoading){u.innerHTML="<div>Loading Game Data...</div>";return}if(g.error){u.innerHTML=`
        ...
        `;return}oe(n)});u.innerHTML="<div>Initializing...</div>";
