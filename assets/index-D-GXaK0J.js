(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(a){if(a.ep)return;a.ep=!0;const i=t(a);fetch(a.href,i)}})();const I={hp:100,maxHp:100,power:5};class N{constructor(e){this.hp=I.hp,this.maxHp=I.maxHp,this.power=I.power,this.interest=33+Math.floor(Math.random()*50),this.traits=e,this.inventory={weapon:null,armor:null,potions:[]}}modifyInterest(e,t){const s=Math.max(.1,(1e3-this.traits.expertise)/1e3),a=(Math.random()*2-1)*t,i=(e+a)*s;this.interest=Math.max(0,Math.min(100,this.interest+i))}equip(e){e.type==="Weapon"?this.inventory.weapon=e:e.type==="Armor"&&(this.inventory.armor=e),this.recalculateStats()}addPotion(e){e.type==="Potion"&&this.inventory.potions.push(e)}recalculateStats(){let e=I.power,t=I.maxHp;this.inventory.weapon&&(e+=this.inventory.weapon.stats.power||0,t+=this.inventory.weapon.stats.maxHp||0),this.inventory.armor&&(e+=this.inventory.armor.stats.power||0,t+=this.inventory.armor.stats.maxHp||0);const s=t-this.maxHp;this.power=e,this.maxHp=t,this.hp+=Math.max(0,s)}}class P{constructor(){this.entries=[]}log(e,t="INFO"){this.entries.push({message:e,level:t,timestamp:Date.now()}),console.log(`[${t}] ${e}`)}debug(e){this.log(e,"DEBUG")}info(e){this.log(e,"INFO")}warn(e){this.log(e,"WARN")}error(e){this.log(e,"ERROR")}}const G=15,Y=3,J={Common:1,Uncommon:2,Rare:3},X=10,R=10,U=["loot_1","loot_2","loot_3","loot_4","loot_5"],ee=32;let te=9;const F=300,Z=r=>[...r].sort(()=>Math.random()-.5),B=(r,e)=>{let t=0;const s=new Map(e.map(h=>[h.id,h])),a=r.map(h=>s.get(h)).filter(Boolean),i=h=>(t++,{...h,instanceId:`${h.id}-${t}`}),o=a.filter(h=>h.cost===null),l=a.filter(h=>h.cost!==null);let c=[];if(o.length>0)for(const h of o)c.push(i(h),i(h),i(h),i(h));c.push(...l.map(i));let m=0;for(;c.length<ee&&o.length>0;)c.push(i(o[m%o.length])),m++;return Z(c)};let z={};async function W(r){try{const e=await fetch(`/rogue-steward/locales/${r}.json`);if(!e.ok)throw new Error(`Could not load ${r}.json`);z=await e.json()}catch(e){console.warn(`Failed to load ${r} translations:`,e),r!=="en"&&await W("en")}}function se(){return navigator.language.split("-")[0]}function n(r,e={}){let s=r.split(".").reduce((a,i)=>a?a[i]:void 0,z);if(!s)return console.warn(`Translation not found for key: ${r}`),r;for(const a in e)s=s.replace(`{${a}}`,String(e[a]));return s}async function ne(){const r=se();await W(r)}var M=(r=>(r.WORKSHOP="workshop",r.HAND_SIZE_INCREASE="hand_size_increase",r.CUSTOM_ENCOUNTERS="custom_encounters",r.PARTY_CONSTRAINTS="party_constraints",r.BOSS_FIGHTS="boss_fights",r))(M||{});const Q=[{feature:"workshop",runThreshold:2,title:()=>n("unlocks.workshop.title"),description:()=>n("unlocks.workshop.description")},{feature:"hand_size_increase",runThreshold:3,title:()=>n("unlocks.hand_size_increase.title"),description:()=>n("unlocks.hand_size_increase.description")},{feature:"boss_fights",runThreshold:5,title:()=>n("unlocks.boss_fights.title"),description:()=>n("unlocks.boss_fights.description")},{feature:"custom_encounters",runThreshold:8,title:()=>n("unlocks.custom_encounters.title"),description:()=>n("unlocks.custom_encounters.description")},{feature:"party_constraints",runThreshold:12,title:()=>n("unlocks.party_constraints.title"),description:()=>n("unlocks.party_constraints.description")}];class ae{constructor(e){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._listeners={},this.init=async()=>{await this._loadGameData()},this.startNewGame=()=>{this.metaManager.incrementAdventurers();const t={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},s=new N(t),a=U,i=B(a,this._allItems),o=this._getHandSize(),l=i.slice(0,o),c=i.slice(o),m=new P;m.info("--- Starting New Game (Run 1) ---"),this.gameState={phase:"DESIGNER_CHOOSING_DIFFICULTY",designer:{balancePoints:0},adventurer:s,unlockedDeck:a,availableDeck:c,hand:l,handSize:o,shopItems:[],offeredLoot:[],feedback:n("game_engine.new_adventurer"),logger:m,run:1,room:1,runEnded:{isOver:!1,reason:""},newlyUnlocked:[]},this._emit("state-change",this.gameState)},this.continueGame=()=>{this.startNewGame()},this.startNewRun=t=>{if(!this.gameState)return;const s=t||this.gameState.run+1;this.metaManager.updateRun(s);const a=this._getHandSize(),i=B(this.gameState.unlockedDeck,this._allItems),o=i.slice(0,a),l=i.slice(a),c=new N(this.gameState.adventurer.traits);c.interest=this.gameState.adventurer.interest,this.gameState.logger.info(`--- Starting Run ${s} ---`),this.gameState={...this.gameState,adventurer:c,phase:"DESIGNER_CHOOSING_DIFFICULTY",availableDeck:l,hand:o,handSize:a,room:1,run:s,feedback:n("game_engine.adventurer_returns"),runEnded:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=t=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(a=>t.includes(a.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;const{choice:a,reason:i}=this._getAdventurerChoice(this.gameState.adventurer,this.gameState.offeredLoot),o=this.gameState.adventurer;let l=this.gameState.hand,c=this.gameState.availableDeck;l.forEach(d=>d.justDrafted=!1);let m=l.filter(d=>!t.includes(d.instanceId));const h=this.gameState.handSize-m.length,f=c.slice(0,h);f.forEach(d=>{d.draftedRoom=this.gameState.room,d.justDrafted=!0});const x=c.slice(h);m.push(...f),a?a.type==="Potion"?o.addPotion(a):o.equip(a):o.interest=Math.max(0,o.interest-10);const v=this.gameState.room+1,_=this.gameState.designer.balancePoints+R;this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:o,feedback:i,availableDeck:x,hand:m,room:v,designer:{balancePoints:_}},this._emit("state-change",this.gameState)},F)},this.runEncounter=t=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_DIFFICULTY")return;let s={...t},a=!1;if(this.gameState.room>0&&this.gameState.room%5===0){a=!0;const i=Math.max(t.enemyPower,20+this.gameState.room),o=Math.max(t.enemyHp,50+this.gameState.room*5);s={enemyCount:1,enemyPower:i,enemyHp:o}}this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.encounter=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.encounter)return;a&&this.gameState.logger.info("--- BOSS FIGHT ---");const{newAdventurer:i,feedback:o}=this._simulateEncounter(this.gameState.adventurer,this.gameState.room,this.gameState.encounter),l=c=>{const m=this.metaManager.checkForUnlocks(this.gameState.run);this.gameState.logger.error(`GAME OVER: ${c}`),this.gameState={...this.gameState,adventurer:i,designer:{balancePoints:this.gameState.designer.balancePoints+R},phase:"RUN_OVER",runEnded:{isOver:!0,reason:c},newlyUnlocked:m},this._emit("state-change",this.gameState)};if(i.hp<=0){l(n("game_engine.adventurer_fell",{room:this.gameState.room,run:this.gameState.run}));return}if(i.interest<=G){l(n("game_engine.adventurer_bored",{room:this.gameState.room,run:this.gameState.run}));return}this.gameState.hand&&this.gameState.hand.length===0?(this.gameState.logger.warn("Your hand is empty! The adventurer must press on without new items."),o.push(n("game_engine.empty_hand")),this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:i,room:this.gameState.room+1,designer:{balancePoints:this.gameState.designer.balancePoints+R},feedback:o,encounter:void 0}):this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",adventurer:i,feedback:o,encounter:void 0},this._emit("state-change",this.gameState)},F)},this.enterWorkshop=()=>{if(!this.gameState)return;if(!this.metaManager.acls.has(M.WORKSHOP)){this.startNewRun();return}const t=this.gameState.run+1,s=this._allItems.filter(a=>a.cost!==null).filter(a=>!this.gameState.unlockedDeck.includes(a.id)).filter(a=>t>=a.minRun);this.gameState={...this.gameState,phase:"SHOP",run:t,room:0,shopItems:Z(s).slice(0,4),runEnded:{isOver:!1,reason:""},feedback:n("game_engine.welcome_to_workshop")},this._emit("state-change",this.gameState)},this.purchaseItem=t=>{if(!this.gameState)return;const s=this._allItems.find(l=>l.id===t);if(!s||s.cost===null||this.gameState.designer.balancePoints<s.cost)return;const a=[...this.gameState.unlockedDeck,t],i=this.gameState.designer.balancePoints-s.cost,o=this.gameState.shopItems.filter(l=>l.id!==t);this.gameState.logger.info(`Purchased ${s.name}.`),this.gameState={...this.gameState,designer:{balancePoints:i},unlockedDeck:a,shopItems:o},this._emit("state-change",this.gameState)},this.showMenu=()=>{this.gameState={...this.gameState||this._getInitialGameState(),phase:"MENU"},this._emit("state-change",this.gameState)},this.metaManager=e}on(e,t){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(t)}_emit(e,t){const s=this._listeners[e];s&&s.forEach(a=>a(t))}_getAdventurerChoice(e,t){var f,x,v,_;const{traits:s,inventory:a}=e;(f=this.gameState)==null||f.logger.debug(`--- Adventurer Decision --- (Offense: ${s.offense}, Risk: ${s.risk})`);const i=((x=a.weapon)==null?void 0:x.stats.power)||0,o=((v=a.armor)==null?void 0:v.stats.maxHp)||0;(_=this.gameState)==null||_.logger.debug(`Current Gear: Weapon Power(${i}), Armor HP(${o})`);const l=d=>{var k,E;let u=(J[d.rarity]||1)*5;switch(d.type){case"Weapon":const w=(d.stats.power||0)-i;if(w<=0&&d.id!==((k=a.weapon)==null?void 0:k.id))return-1;u+=w*(s.offense/10),w>0&&(u+=w*(s.expertise/10));const $=d.stats.maxHp||0;$<0&&(u+=$*(100-s.risk)/20);break;case"Armor":const b=(d.stats.maxHp||0)-o;if(b<=0&&d.id!==((E=a.armor)==null?void 0:E.id))return-1;u+=b*(100-s.offense)/10,b>0&&(u+=b*(s.expertise/10));const H=d.stats.power||0;H>0&&(u+=H*(s.offense/15));const T=d.stats.power||0;T<0&&(u+=T*(s.risk/10));break;case"Potion":const y=e.hp/e.maxHp;u+=10*(100-s.risk)/100,y<.7&&(u+=20*(1-y)),u+=5*(s.expertise/100),a.potions.length>=Y&&(u*=.1);break}return u+Math.random()},c=t.map(d=>({item:d,score:l(d)})).filter(d=>d.score>0);if(c.sort((d,u)=>u.score-d.score),c.length===0||c[0].score<X)return{choice:null,reason:n("game_engine.adventurer_declines_offer")};const m=c[0].item,h=n("game_engine.adventurer_accepts_offer",{itemName:m.name});return{choice:m,reason:h}}_simulateEncounter(e,t,s){var f,x,v,_,d,u,k,E,w,$;(f=this.gameState)==null||f.logger.info(`--- Encounter: Room ${t} ---`);const a=[];let i=0,o=0;const l=e.hp;for(let b=0;b<s.enemyCount;b++){(x=this.gameState)==null||x.logger.info(`Adventurer encounters enemy ${b+1}/${s.enemyCount}.`);const H=e.hp/e.maxHp,T=1-e.traits.risk/120;if(H<T&&e.inventory.potions.length>0){const S=e.inventory.potions.shift();if(S){const C=S.stats.hp||0;e.hp=Math.min(e.maxHp,e.hp+C),a.push(n("game_engine.adventurer_drinks_potion",{potionName:S.name})),(v=this.gameState)==null||v.logger.info(`Adventurer used ${S.name} and recovered ${C} HP.`)}}let y=s.enemyHp;for(;y>0&&e.hp>0;){const S=Math.min(.95,.75+e.traits.expertise/500+e.traits.offense/1e3);if(Math.random()<S){const L=e.power;y-=L,(_=this.gameState)==null||_.logger.debug(`Adventurer hits for ${L} damage.`)}else(d=this.gameState)==null||d.logger.debug("Adventurer misses.");if(y<=0){(u=this.gameState)==null||u.logger.info("Enemy defeated."),o++;break}const C=Math.max(.4,.75-e.traits.expertise/500-(100-e.traits.offense)/1e3);if(Math.random()<C){const L=(((k=e.inventory.armor)==null?void 0:k.stats.maxHp)||0)/10,D=Math.max(1,s.enemyPower-L);i+=D,e.hp-=D,(E=this.gameState)==null||E.logger.debug(`Enemy hits for ${D} damage.`)}else(w=this.gameState)==null||w.logger.debug("Enemy misses.")}if(e.hp<=0){($=this.gameState)==null||$.logger.warn("Adventurer has been defeated.");break}}let c;const h=(l-e.hp)/e.maxHp;return h>.7?(c=n("game_engine.too_close_for_comfort"),e.modifyInterest(-15,5)):h>.4?(c=n("game_engine.great_battle"),e.modifyInterest(10,5)):o>3&&e.traits.offense>60?(c=n("game_engine.easy_fight"),e.modifyInterest(5,5)):(c=n("game_engine.worthy_challenge"),e.modifyInterest(-2,3)),a.push(c),e.hp>0&&o===s.enemyCount&&(e.traits.expertise+=1),{newAdventurer:e,feedback:a,totalDamageTaken:i}}getAdventurerEndRunDecision(){if(!this.gameState)return"retire";const{interest:e}=this.gameState.adventurer,t=e-G,s=(Math.random()-.5)*20;return t+s>0?"continue":"retire"}handleEndOfRun(e){if(this.gameState){if(e==="retire"){this.showMenu();return}this.enterWorkshop()}}_getInitialGameState(){const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},t=new N(e);return{phase:"MENU",designer:{balancePoints:0},adventurer:t,unlockedDeck:U,availableDeck:[],hand:[],handSize:this._getHandSize(),shopItems:[],offeredLoot:[],feedback:"",logger:new P,run:0,room:0,runEnded:{isOver:!1,reason:""},newlyUnlocked:[]}}_getHandSize(){return this.metaManager.acls.has(M.HAND_SIZE_INCREASE)?12:te}isWorkshopUnlocked(){return this.metaManager.acls.has(M.WORKSHOP)}async _loadGameData(){try{const e=await fetch("/rogue-steward/game/items.json");if(!e.ok)throw new Error(n("global.error_loading_items",{statusText:e.statusText}));this._allItems=await e.json()}catch(e){this.error=e.message||n("global.unknown_error"),this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}const re=r=>{if(!r)return n("global.initializing");switch(r.phase){case"AWAITING_ADVENTURER_CHOICE":return n("main.adventurer_considering_offer");case"AWAITING_ENCOUNTER_FEEDBACK":return n("main.adventurer_facing_encounter");default:return n("global.loading")}},ie=r=>{switch(r.phase){case"DESIGNER_CHOOSING_LOOT":return'<div class="lg:col-span-3"><loot-choice-panel></loot-choice-panel></div>';case"DESIGNER_CHOOSING_DIFFICULTY":return'<div class="lg:col-span-3"><battle-panel></battle-panel></div>';case"AWAITING_ADVENTURER_CHOICE":case"AWAITING_ENCOUNTER_FEEDBACK":return`<div class="lg:col-span-3"><loading-indicator text="${re(r)}"></loading-indicator></div>`;default:return`<div class="lg:col-span-3"><div>${n("main.unhandled_game_phase",{phase:r.phase})}</div></div>`}},oe=(r,e,t)=>{if(!e){r.innerHTML=`<div>${n("global.loading")}</div>`;return}if(e.phase==="MENU"){r.innerHTML="<menu-screen></menu-screen>";return}if(e.phase==="SHOP"){r.innerHTML="<workshop-screen></workshop-screen>";const l=document.querySelector("workshop-screen");l&&(l.items=e.shopItems,l.balancePoints=e.designer.balancePoints);return}const s=e.runEnded.isOver?`<run-ended-screen
    ...
    `,e.runEnded.isOver){const l=document.querySelector("run-ended-screen");l&&(l.newlyUnlocked=e.newlyUnlocked,l.setDecision(t.getAdventurerEndRunDecision()))}const a=document.querySelector("adventurer-status");a&&(a.adventurer=e.adventurer,a.metaState=t.metaManager.metaState);const i=document.querySelector("loot-choice-panel");i&&(i.choices=e.hand,i.disabled=!1);const o=document.querySelector("log-panel");o&&(o.logger=e.logger,o.traits=e.adventurer.traits)},j="rogue-steward-meta";class K{constructor(){this._metaState=this._load()}get metaState(){return this._metaState}get acls(){return new Set(this._metaState.unlockedFeatures)}checkForUnlocks(e){const t=[];for(const s of Q)e>=s.runThreshold&&!this._metaState.unlockedFeatures.includes(s.feature)&&(this._metaState.unlockedFeatures.push(s.feature),t.push(s.feature));return t.length>0&&this.save(),t}incrementAdventurers(){this._metaState.adventurers=(this._metaState.adventurers||0)+1,this.save()}updateRun(e){e>this._metaState.highestRun&&(this._metaState.highestRun=e,this.save())}_load(){try{const e=localStorage.getItem(j);if(e){const t=JSON.parse(e);if(typeof t.highestRun=="number"&&Array.isArray(t.unlockedFeatures))return typeof t.adventurers!="number"&&(t.adventurers=1),t}}catch(e){console.error("Failed to load meta state:",e)}return this._defaultState()}save(){try{localStorage.setItem(j,JSON.stringify(this._metaState))}catch(e){console.error("Failed to save meta state:",e)}}reset(){this._metaState=this._defaultState(),this.save()}_defaultState(){return{highestRun:0,unlockedFeatures:[],adventurers:0}}}const le=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-red-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd" /></svg>',ce=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-blue-400" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M240-400q0 68.92 36.77 126.58 36.77 57.65 98.85 86.5-7.93-11.93-11.77-24.77Q360-224.54 360-238q0-23.54 9.31-44.62 9.31-21.07 26.15-37.92L480-403.85l85.31 83.31q16.84 16.85 25.77 37.92Q600-261.54 600-238q0 13.46-3.85 26.31-3.84 12.84-11.77 24.77 62.08-28.85 98.85-86.5Q720-331.08 720-400q0-50-18.5-94.5T648-574q-20 13-42 19.5t-45 6.5q-62.77 0-108.27-41-45.5-41-51.73-102.54-39 32.23-69 67.73-30 35.5-50.5 72.77-20.5 37.27-31 75.27-10.5 38-10.5 75.77Zm240 52-57 56q-11 11-17 25t-6 29q0 32 23.5 55t56.5 23q33 0 56.5-23t23.5-55q0-16-6-29.5T537-292l-57-56Zm-40-420.46V-708q0 50.92 35.04 85.46Q510.08-588 561-588q18.77 0 36.58-6.35 17.8-6.34 33.65-18.27l17.23-13.53q52.46 38.92 82 98.92T760-400q0 117.08-81.46 198.54T480-120q-117.08 0-198.54-81.46T200-400q0-102.85 64.58-201.15Q329.15-699.46 440-768.46Z"/></svg>',de=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-amber-400" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M249.23-80H144.62Q117-80 98.5-98.5 80-117 80-144.62v-104.61h40v104.61q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92h104.61v40Zm461.54 0v-40h104.61q10.77 0 17.7-6.92 6.92-6.93 6.92-17.7v-104.61h40v104.61q0 27.62-18.5 46.12Q843-80 815.38-80H710.77ZM480-250.77q-106.92 0-192.12-60.61Q202.69-372 159.23-480q43.46-108 128.65-168.62 85.2-60.61 192.12-60.61t192.12 60.61Q757.31-588 800.77-480q-43.46 108-128.65 168.62-85.2 60.61-192.12 60.61Zm0-40q91.08 0 163.31-49.54T756.85-480q-41.31-90.15-113.54-139.69-72.23-49.54-163.31-49.54t-163.31 49.54Q244.46-570.15 203.15-480q41.31 90.15 113.54 139.69 72.23 49.54 163.31 49.54Zm0-74.61q48 0 81.31-33.31T594.62-480q0-48-33.31-81.31T480-594.62q-48 0-81.31 33.31T365.38-480q0 48 33.31 81.31T480-365.38Zm0-40q-31.15 0-52.88-21.74-21.74-21.73-21.74-52.88 0-31.15 21.74-52.88 21.73-21.74 52.88-21.74 31.15 0 52.88 21.74 21.74 21.73 21.74 52.88 0 31.15-21.74 52.88-21.73 21.74-52.88 21.74ZM80-710.77v-104.61Q80-843 98.5-861.5 117-880 144.62-880h104.61v40H144.62q-10.77 0-17.7 6.92-6.92 6.93-6.92 17.7v104.61H80Zm760 0v-104.61q0-10.77-6.92-17.7-6.93-6.92-17.7-6.92H710.77v-40h104.61q27.62 0 46.12 18.5Q880-843 880-815.38v104.61h-40ZM480-480Z"/></svg>',he=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M753.54-132.15 631.15-254.31l-88 88-11.07-11.07q-18.39-18.39-18.39-45.47 0-27.07 18.39-45.46l163.61-163.61q18.39-18.39 45.46-18.39 27.08 0 45.47 18.39l11.07 11.07-88 88 122.16 122.39q9.69 9.69 9.69 22.61 0 12.93-9.69 22.62l-33.08 33.08q-9.69 9.69-22.62 9.69-12.92 0-22.61-9.69ZM840-740.92 398.31-298.46l29.61 29.38q18.39 18.39 18.39 45.46 0 27.08-18.39 45.47l-11.07 11.07-88-88-122.39 122.16q-9.69 9.69-22.61 9.69-12.93 0-22.62-9.69L128.15-166q-9.69-9.69-9.69-22.62 0-12.92 9.69-22.61l122.16-122.39-88-88 11.07-11.07q18.39-18.39 45.47-18.39 27.07 0 45.46 18.39l30.15 30.38L736.92-844H840v103.08ZM334-583l23.23-23.77 23-24-23 24L334-583Zm-28.31 28.54L120-740.92V-844h103.08L408-658.31l-27.77 27.54L207-804h-47v47l174 174-28.31 28.54ZM370-327l430-430v-47h-47L323-374l47 47Zm0 0-23.23-23.77L323-374l23.77 23.23L370-327Z"/></svg>',ue=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-121.54q-120.54-35.77-200.27-146.04Q200-377.85 200-516v-216.31l280-104.61 280 104.61V-516q0 138.15-79.73 248.42Q600.54-157.31 480-121.54Zm0-42.46q104-33 172-132t68-220v-189l-240-89.23L240-705v189q0 121 68 220t172 132Zm0-315.23Z"/></svg>',me=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M200-160q-25.54 0-36.31-22.81-10.77-22.81 5.08-42.57L400-506.15V-760h-55.38q-8.5 0-14.25-5.76t-5.75-14.27q0-8.51 5.75-14.24t14.25-5.73h270.76q8.5 0 14.25 5.76t5.75 14.27q0 8.51-5.75 14.24T615.38-760H560v253.85l231.23 280.77q15.85 19.76 5.08 42.57T760-160H200Zm80-80h400L544-400H416L280-240Zm-80 40h560L520-492v-268h-80v268L200-200Zm280-280Z"/></svg>';class pe extends HTMLElement{constructor(){super(),this._adventurer=null,this._metaState=null}set adventurer(e){this._adventurer=e,this.render()}set metaState(e){this._metaState=e}get adventurer(){return this._adventurer}connectedCallback(){this.render()}render(){var s;if(!this._adventurer){this.innerHTML="";return}const e=((s=this._metaState)==null?void 0:s.adventurers)||1,t=this._adventurer.hp/this._adventurer.maxHp*100;this.innerHTML=`
            <div class="bg-brand-surface p-6 pixel-corners shadow-xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${de()} <span class=" text-lg">${n("adventurer_status.interest")}</span></div>
        ...
        `}}customElements.define("game-stats",xe);class ve extends HTMLElement{constructor(){super(),this._text="Loading..."}static get observedAttributes(){return["text"]}attributeChangedCallback(e,t,s){e==="text"&&(this._text=s,this.render())}connectedCallback(){this.render()}render(){this.innerHTML=`
