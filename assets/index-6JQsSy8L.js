(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(a){if(a.ep)return;a.ep=!0;const r=t(a);fetch(a.href,r)}})();const H={hp:100,maxHp:100,power:5};class R{constructor(e,t){this.hp=H.hp,this.maxHp=H.maxHp,this.power=H.power,this.interest=33+Math.floor(Math.random()*50),this.traits=e,this.inventory={weapon:null,armor:null,potions:[]},this.logger=t}modifyInterest(e,t){const s=Math.max(.1,(1e3-this.traits.expertise)/1e3),a=(Math.random()*2-1)*t,r=(e+a)*s,o=this.interest;this.interest=Math.max(0,Math.min(100,this.interest+r)),this.logger.debug(`Interest changed from ${o.toFixed(1)} to ${this.interest.toFixed(1)} (Base: ${e}, Rand: ${a.toFixed(1)}, Total: ${r.toFixed(1)})`)}equip(e){e.type==="Weapon"?this.inventory.weapon=e:e.type==="Armor"&&(this.inventory.armor=e),this.recalculateStats()}addPotion(e){e.type==="Potion"&&this.inventory.potions.push(e)}recalculateStats(){let e=H.power,t=H.maxHp;this.inventory.weapon&&(e+=this.inventory.weapon.stats.power||0,t+=this.inventory.weapon.stats.maxHp||0),this.inventory.armor&&(e+=this.inventory.armor.stats.power||0,t+=this.inventory.armor.stats.maxHp||0);const s=t-this.maxHp;this.power=e,this.maxHp=t,this.hp+=Math.max(0,s)}}class G{constructor(){this.entries=[]}log(e,t="INFO"){this.entries.push({message:e,level:t,timestamp:Date.now()}),console.log(`[${t}] ${e}`)}debug(e){this.log(e,"DEBUG")}info(e){this.log(e,"INFO")}warn(e){this.log(e,"WARN")}error(e){this.log(e,"ERROR")}}const U=15,X=99,ee={Common:1,Uncommon:2,Rare:3},te=10,A=10,F=["loot_1","loot_2","loot_3","loot_4","loot_5"],se=4,j=300,B=i=>`${i}_${Math.random().toString(36).substr(2,9)}`,W=i=>{const e=[...i];for(let t=e.length-1;t>0;t--){const s=Math.floor(Math.random()*(t+1));[e[t],e[s]]=[e[s],e[t]]}return e},Z=(i,e)=>{const t=e.filter(r=>i.includes(r.id)),s=[];t.filter(r=>r.cost===null).forEach(r=>{s.push({...r,instanceId:B(r.id)})});const a=t.filter(r=>r.cost!==null);for(;s.length<4&&a.length>0;){const r=Math.floor(Math.random()*a.length),o=a[r];s.push({...o,instanceId:B(o.id)}),a.splice(r,1)}return W(s)};let Q={};async function V(i){try{const e=await fetch(`/rogue-steward/locales/${i}.json`);if(!e.ok)throw new Error(`Could not load ${i}.json`);Q=await e.json()}catch(e){console.warn(`Failed to load ${i} translations:`,e),i!=="en"&&await V("en")}}function ne(){return navigator.language.split("-")[0]}function n(i,e={}){let s=i.split(".").reduce((a,r)=>a?a[r]:void 0,Q);if(!s)return console.warn(`Translation not found for key: ${i}`),i;for(const a in e)s=s.replace(`{${a}}`,String(e[a]));return s}async function ae(){const i=ne();await V(i)}var D=(i=>(i.WORKSHOP="workshop",i.HAND_SIZE_INCREASE="hand_size_increase",i.CUSTOM_ENCOUNTERS="custom_encounters",i.PARTY_CONSTRAINTS="party_constraints",i.BOSS_FIGHTS="boss_fights",i))(D||{});const Y=[{feature:"workshop",runThreshold:2,title:()=>n("unlocks.workshop.title"),description:()=>n("unlocks.workshop.description")},{feature:"hand_size_increase",runThreshold:3,title:()=>n("unlocks.hand_size_increase.title"),description:()=>n("unlocks.hand_size_increase.description")},{feature:"boss_fights",runThreshold:5,title:()=>n("unlocks.boss_fights.title"),description:()=>n("unlocks.boss_fights.description")},{feature:"custom_encounters",runThreshold:8,title:()=>n("unlocks.custom_encounters.title"),description:()=>n("unlocks.custom_encounters.description")},{feature:"party_constraints",runThreshold:12,title:()=>n("unlocks.party_constraints.title"),description:()=>n("unlocks.party_constraints.description")}];class re{constructor(e){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._listeners={},this.init=async()=>{await this._loadGameData()},this.startNewGame=()=>{this.metaManager.incrementAdventurers();const t={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},s=new G,a=new R(t,s),r=F,o=Z(r,this._allItems),l=this._getHandSize(),c=o.slice(0,l),m=o.slice(l);s.info("--- Starting New Game (Run 1) ---"),this.gameState={phase:"DESIGNER_CHOOSING_DIFFICULTY",designer:{balancePoints:0},adventurer:a,unlockedDeck:r,availableDeck:m,hand:c,handSize:l,shopItems:[],offeredLoot:[],feedback:n("game_engine.new_adventurer"),logger:s,run:1,room:1,runEnded:{isOver:!1,reason:""},newlyUnlocked:[]},this._emit("state-change",this.gameState)},this.continueGame=()=>{this.startNewGame()},this.startNewRun=t=>{if(!this.gameState)return;const s=t||this.gameState.run+1;this.metaManager.updateRun(s);const a=this._getHandSize(),r=Z(this.gameState.unlockedDeck,this._allItems),o=r.slice(0,a),l=r.slice(a),c=new R(this.gameState.adventurer.traits,this.gameState.logger);c.interest=this.gameState.adventurer.interest,this.gameState.logger.info(`--- Starting Run ${s} ---`),this.gameState={...this.gameState,adventurer:c,phase:"DESIGNER_CHOOSING_DIFFICULTY",availableDeck:l,hand:o,handSize:a,room:1,run:s,feedback:n("game_engine.adventurer_returns"),runEnded:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=t=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(a=>t.includes(a.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;const{choice:a,reason:r}=this._getAdventurerChoice(this.gameState.adventurer,this.gameState.offeredLoot),o=this.gameState.adventurer;let l=this.gameState.hand,c=this.gameState.availableDeck;l.forEach(g=>g.justDrafted=!1);let m=l.filter(g=>!t.includes(g.instanceId));const f=this.gameState.handSize-m.length,b=c.slice(0,f);b.forEach(g=>{g.draftedRoom=this.gameState.room,g.justDrafted=!0});const v=c.slice(f);m.push(...b),a?a.type==="Potion"?o.addPotion(a):o.equip(a):o.interest=Math.max(0,o.interest-10);const _=this.gameState.room+1,w=this.gameState.designer.balancePoints+A;this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:o,feedback:r,availableDeck:v,hand:m,room:_,designer:{balancePoints:w}},this._emit("state-change",this.gameState)},j)},this.runEncounter=t=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_DIFFICULTY")return;let s={...t},a=!1;if(this.gameState.room>0&&this.gameState.room%5===0){a=!0;const r=Math.max(t.enemyPower,20+this.gameState.room),o=Math.max(t.enemyHp,50+this.gameState.room*5);s={enemyCount:1,enemyPower:r,enemyHp:o}}this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.encounter=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.encounter)return;a&&this.gameState.logger.info("--- BOSS FIGHT ---");const{newAdventurer:r,feedback:o}=this._simulateEncounter(this.gameState.adventurer,this.gameState.room,this.gameState.encounter),l=c=>{const m=this.metaManager.checkForUnlocks(this.gameState.run);this.gameState.logger.error(`GAME OVER: ${c}`),this.gameState={...this.gameState,adventurer:r,designer:{balancePoints:this.gameState.designer.balancePoints+A},phase:"RUN_OVER",runEnded:{isOver:!0,reason:c},newlyUnlocked:m},this._emit("state-change",this.gameState)};if(r.hp<=0){l(n("game_engine.adventurer_fell",{room:this.gameState.room,run:this.gameState.run}));return}if(r.interest<=U){l(n("game_engine.adventurer_bored",{room:this.gameState.room,run:this.gameState.run}));return}this.gameState.hand&&this.gameState.hand.length===0?(this.gameState.logger.warn("Your hand is empty! The adventurer must press on without new items."),o.push(n("game_engine.empty_hand")),this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:r,room:this.gameState.room+1,designer:{balancePoints:this.gameState.designer.balancePoints+A},feedback:o,encounter:void 0}):this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",adventurer:r,feedback:o,encounter:void 0},this._emit("state-change",this.gameState)},j)},this.enterWorkshop=()=>{if(!this.gameState)return;if(!this.metaManager.acls.has(D.WORKSHOP)){this.startNewRun();return}const t=this.gameState.run+1,s=this._allItems.filter(a=>a.cost!==null).filter(a=>!this.gameState.unlockedDeck.includes(a.id));this.gameState={...this.gameState,phase:"SHOP",run:t,room:0,shopItems:W(s).slice(0,4),runEnded:{isOver:!1,reason:""},feedback:n("game_engine.welcome_to_workshop")},this._emit("state-change",this.gameState)},this.purchaseItem=t=>{if(!this.gameState)return;const s=this._allItems.find(l=>l.id===t);if(!s||s.cost===null||this.gameState.designer.balancePoints<s.cost)return;const a=[...this.gameState.unlockedDeck,t],r=this.gameState.designer.balancePoints-s.cost,o=this.gameState.shopItems.filter(l=>l.id!==t);this.gameState.logger.info(`Purchased ${s.name}.`),this.gameState={...this.gameState,designer:{balancePoints:r},unlockedDeck:a,shopItems:o},this._emit("state-change",this.gameState)},this.showMenu=()=>{this.gameState={...this.gameState||this._getInitialGameState(),phase:"MENU"},this._emit("state-change",this.gameState)},this.metaManager=e}on(e,t){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(t)}_emit(e,t){const s=this._listeners[e];s&&s.forEach(a=>a(t))}_getAdventurerChoice(e,t){var b,v,_,w,g;const{traits:s,inventory:a}=e;(b=this.gameState)==null||b.logger.debug(`--- Adventurer Decision --- (Offense: ${s.offense}, Risk: ${s.risk})`);const r=((v=a.weapon)==null?void 0:v.stats.power)||0,o=((_=a.armor)==null?void 0:_.stats.maxHp)||0;(w=this.gameState)==null||w.logger.debug(`Current Gear: Weapon Power(${r}), Armor HP(${o})`);const l=d=>{var k,E;let h=(ee[d.rarity]||1)*5;switch(d.type){case"Weapon":const y=(d.stats.power||0)-r;if(y<=0&&d.id!==((k=a.weapon)==null?void 0:k.id))return-1;h+=y*(s.offense/10),y>0&&(h+=y*(s.expertise/10));const I=d.stats.maxHp||0;I<0&&(h+=I*(100-s.risk)/20);break;case"Armor":const x=(d.stats.maxHp||0)-o;if(x<=0&&d.id!==((E=a.armor)==null?void 0:E.id))return-1;h+=x*(100-s.offense)/10,x>0&&(h+=x*(s.expertise/10));const T=d.stats.power||0;T>0&&(h+=T*(s.offense/15));const C=d.stats.power||0;C<0&&(h+=C*(s.risk/10));break;case"Potion":const S=e.hp/e.maxHp;h+=10*(100-s.risk)/100,S<.7&&(h+=20*(1-S)),h+=5*(s.expertise/100),a.potions.length>=X&&(h*=.1);break}return h},c=t.map(d=>({item:d,score:l(d)})).filter(d=>d.score>0);if(c.sort((d,h)=>h.score-d.score),c.length===0||c[0].score<te)return e.modifyInterest(-15,10),{choice:null,reason:n("game_engine.adventurer_declines_offer")};const m=c[0].item;(g=this.gameState)==null||g.logger.debug(`Adventurer chooses: ${m.name} (Score: ${c[0].score.toFixed(1)})`),c[0].score>60?e.modifyInterest(15,5):c[0].score>30?e.modifyInterest(10,8):e.modifyInterest(5,5);const f=n("game_engine.adventurer_accepts_offer",{itemName:m.name});return{choice:m,reason:f}}_simulateEncounter(e,t,s){var b,v,_,w,g,d,h,k,E,y,I;(b=this.gameState)==null||b.logger.info(`--- Encounter: Room ${t} ---`);const a=[];let r=0,o=0;const l=e.hp;for(let x=0;x<s.enemyCount;x++){(v=this.gameState)==null||v.logger.info(`Adventurer encounters enemy ${x+1}/${s.enemyCount}.`);const T=e.hp/e.maxHp,C=1-e.traits.risk/120;if(T<C&&e.inventory.potions.length>0){const $=e.inventory.potions.shift();if($){const L=$.stats.hp||0;e.hp=Math.min(e.maxHp,e.hp+L),a.push(n("game_engine.adventurer_drinks_potion",{potionName:$.name})),(_=this.gameState)==null||_.logger.info(`Adventurer used ${$.name} and recovered ${L} HP.`)}}let S=s.enemyHp;for(;S>0&&e.hp>0;){const $=Math.min(.95,.75+e.traits.expertise/500+e.traits.offense/1e3);if(Math.random()<$){const M=e.power;S-=M,(w=this.gameState)==null||w.logger.debug(`Adventurer hits for ${M} damage.`)}else(g=this.gameState)==null||g.logger.debug("Adventurer misses.");if(S<=0){(d=this.gameState)==null||d.logger.info("Enemy defeated."),o++;break}const L=Math.max(.4,.75-e.traits.expertise/500-(100-e.traits.offense)/1e3);if(Math.random()<L){const M=(((h=e.inventory.armor)==null?void 0:h.stats.maxHp)||0)/10,N=Math.max(1,s.enemyPower-M);r+=N,e.hp-=N,(k=this.gameState)==null||k.logger.debug(`Enemy hits for ${N} damage.`)}else(E=this.gameState)==null||E.logger.debug("Enemy misses.")}if(e.hp<=0){(y=this.gameState)==null||y.logger.warn("Adventurer has been defeated.");break}}let c;const m=l-e.hp,f=m/e.maxHp;return(I=this.gameState)==null||I.logger.debug(`hpLost: ${m}, hpLostRatio: ${f.toFixed(2)}`),f>.7?(c=n("game_engine.too_close_for_comfort"),e.modifyInterest(-15,5)):f>.4?(c=n("game_engine.great_battle"),e.modifyInterest(10,5)):o>3&&e.traits.offense>60?(c=n("game_engine.easy_fight"),e.modifyInterest(0,5)):(c=n("game_engine.worthy_challenge"),e.modifyInterest(-2,3)),a.push(c),e.hp>0&&o===s.enemyCount&&(e.traits.expertise+=1),{newAdventurer:e,feedback:a,totalDamageTaken:r}}getAdventurerEndRunDecision(){if(!this.gameState)return"retire";const{interest:e}=this.gameState.adventurer,t=e-U,s=(Math.random()-.5)*20;return t+s>0?"continue":"retire"}handleEndOfRun(e){if(this.gameState){if(e==="retire"){this.showMenu();return}this.enterWorkshop()}}_getInitialGameState(){const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},t=new G,s=new R(e,t);return{phase:"MENU",designer:{balancePoints:0},adventurer:s,unlockedDeck:F,availableDeck:[],hand:[],handSize:this._getHandSize(),shopItems:[],offeredLoot:[],feedback:"",logger:t,run:0,room:0,runEnded:{isOver:!1,reason:""},newlyUnlocked:[]}}_getHandSize(){return this.metaManager.acls.has(D.HAND_SIZE_INCREASE)?12:se}isWorkshopUnlocked(){return this.metaManager.acls.has(D.WORKSHOP)}async _loadGameData(){try{const e=await fetch("/rogue-steward/game/items.json");if(!e.ok)throw new Error(n("global.error_loading_items",{statusText:e.statusText}));this._allItems=await e.json()}catch(e){this.error=e.message||n("global.unknown_error"),this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}const ie=i=>{if(!i)return n("global.initializing");switch(i.phase){case"AWAITING_ADVENTURER_CHOICE":return n("main.adventurer_considering_offer");case"AWAITING_ENCOUNTER_FEEDBACK":return n("main.adventurer_facing_encounter");default:return n("global.loading")}},oe=i=>{switch(i.phase){case"DESIGNER_CHOOSING_LOOT":return'<div class="lg:col-span-3"><loot-choice-panel></loot-choice-panel></div>';case"DESIGNER_CHOOSING_DIFFICULTY":return'<div class="lg:col-span-3"><battle-panel></battle-panel></div>';case"AWAITING_ADVENTURER_CHOICE":case"AWAITING_ENCOUNTER_FEEDBACK":return`<div class="lg:col-span-3"><loading-indicator text="${ie(i)}"></loading-indicator></div>`;default:return`<div class="lg:col-span-3"><div>${n("main.unhandled_game_phase",{phase:i.phase})}</div></div>`}},le=(i,e,t)=>{if(!e){i.innerHTML=`<div>${n("global.loading")}</div>`;return}if(e.phase==="MENU"){i.innerHTML="<menu-screen></menu-screen>";return}if(e.phase==="SHOP"){i.innerHTML="<workshop-screen></workshop-screen>";const l=document.querySelector("workshop-screen");l&&(l.items=e.shopItems,l.balancePoints=e.designer.balancePoints);return}const s=e.runEnded.isOver?`<run-ended-screen
    ...
    `,e.runEnded.isOver){const l=document.querySelector("run-ended-screen");l&&(l.newlyUnlocked=e.newlyUnlocked,l.setDecision(t.getAdventurerEndRunDecision()))}const a=document.querySelector("adventurer-status");a&&(a.metaState=t.metaManager.metaState,a.adventurer=e.adventurer);const r=document.querySelector("loot-choice-panel");r&&(r.choices=e.hand,r.disabled=!1);const o=document.querySelector("log-panel");o&&(o.logger=e.logger,o.traits=e.adventurer.traits)},z="rogue-steward-meta";class K{constructor(){this._metaState=this._load()}get metaState(){return this._metaState}get acls(){return new Set(this._metaState.unlockedFeatures)}checkForUnlocks(e){const t=[];for(const s of Y)e>=s.runThreshold&&!this._metaState.unlockedFeatures.includes(s.feature)&&(this._metaState.unlockedFeatures.push(s.feature),t.push(s.feature));return t.length>0&&this.save(),t}incrementAdventurers(){this._metaState.adventurers=(this._metaState.adventurers||0)+1,this.save()}updateRun(e){e>this._metaState.highestRun&&(this._metaState.highestRun=e,this.save())}_load(){try{const e=localStorage.getItem(z);if(e){const t=JSON.parse(e);if(typeof t.highestRun=="number"&&Array.isArray(t.unlockedFeatures))return typeof t.adventurers!="number"&&(t.adventurers=1),t}}catch(e){console.error("Failed to load meta state:",e)}return this._defaultState()}save(){try{localStorage.setItem(z,JSON.stringify(this._metaState))}catch(e){console.error("Failed to save meta state:",e)}}reset(){this._metaState=this._defaultState(),this.save()}_defaultState(){return{highestRun:0,unlockedFeatures:[],adventurers:0}}}const ce=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-red-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd" /></svg>',de=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-blue-400" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M240-400q0 68.92 36.77 126.58 36.77 57.65 98.85 86.5-7.93-11.93-11.77-24.77Q360-224.54 360-238q0-23.54 9.31-44.62 9.31-21.07 26.15-37.92L480-403.85l85.31 83.31q16.84 16.85 25.77 37.92Q600-261.54 600-238q0 13.46-3.85 26.31-3.84 12.84-11.77 24.77 62.08-28.85 98.85-86.5Q720-331.08 720-400q0-50-18.5-94.5T648-574q-20 13-42 19.5t-45 6.5q-62.77 0-108.27-41-45.5-41-51.73-102.54-39 32.23-69 67.73-30 35.5-50.5 72.77-20.5 37.27-31 75.27-10.5 38-10.5 75.77Zm240 52-57 56q-11 11-17 25t-6 29q0 32 23.5 55t56.5 23q33 0 56.5-23t23.5-55q0-16-6-29.5T537-292l-57-56Zm-40-420.46V-708q0 50.92 35.04 85.46Q510.08-588 561-588q18.77 0 36.58-6.35 17.8-6.34 33.65-18.27l17.23-13.53q52.46 38.92 82 98.92T760-400q0 117.08-81.46 198.54T480-120q-117.08 0-198.54-81.46T200-400q0-102.85 64.58-201.15Q329.15-699.46 440-768.46Z"/></svg>',he=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-amber-400" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M249.23-80H144.62Q117-80 98.5-98.5 80-117 80-144.62v-104.61h40v104.61q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92h104.61v40Zm461.54 0v-40h104.61q10.77 0 17.7-6.92 6.92-6.93 6.92-17.7v-104.61h40v104.61q0 27.62-18.5 46.12Q843-80 815.38-80H710.77ZM480-250.77q-106.92 0-192.12-60.61Q202.69-372 159.23-480q43.46-108 128.65-168.62 85.2-60.61 192.12-60.61t192.12 60.61Q757.31-588 800.77-480q-43.46 108-128.65 168.62-85.2 60.61-192.12 60.61Zm0-40q91.08 0 163.31-49.54T756.85-480q-41.31-90.15-113.54-139.69-72.23-49.54-163.31-49.54t-163.31 49.54Q244.46-570.15 203.15-480q41.31 90.15 113.54 139.69 72.23 49.54 163.31 49.54Zm0-74.61q48 0 81.31-33.31T594.62-480q0-48-33.31-81.31T480-594.62q-48 0-81.31 33.31T365.38-480q0 48 33.31 81.31T480-365.38Zm0-40q-31.15 0-52.88-21.74-21.74-21.73-21.74-52.88 0-31.15 21.74-52.88 21.73-21.74 52.88-21.74 31.15 0 52.88 21.74 21.74 21.73 21.74 52.88 0 31.15-21.74 52.88-21.73 21.74-52.88 21.74ZM80-710.77v-104.61Q80-843 98.5-861.5 117-880 144.62-880h104.61v40H144.62q-10.77 0-17.7 6.92-6.92 6.93-6.92 17.7v104.61H80Zm760 0v-104.61q0-10.77-6.92-17.7-6.93-6.92-17.7-6.92H710.77v-40h104.61q27.62 0 46.12 18.5Q880-843 880-815.38v104.61h-40ZM480-480Z"/></svg>',ue=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M753.54-132.15 631.15-254.31l-88 88-11.07-11.07q-18.39-18.39-18.39-45.47 0-27.07 18.39-45.46l163.61-163.61q18.39-18.39 45.46-18.39 27.08 0 45.47 18.39l11.07 11.07-88 88 122.16 122.39q9.69 9.69 9.69 22.61 0 12.93-9.69 22.62l-33.08 33.08q-9.69 9.69-22.62 9.69-12.92 0-22.61-9.69ZM840-740.92 398.31-298.46l29.61 29.38q18.39 18.39 18.39 45.46 0 27.08-18.39 45.47l-11.07 11.07-88-88-122.39 122.16q-9.69 9.69-22.61 9.69-12.93 0-22.62-9.69L128.15-166q-9.69-9.69-9.69-22.62 0-12.92 9.69-22.61l122.16-122.39-88-88 11.07-11.07q18.39-18.39 45.47-18.39 27.07 0 45.46 18.39l30.15 30.38L736.92-844H840v103.08ZM334-583l23.23-23.77 23-24-23 24L334-583Zm-28.31 28.54L120-740.92V-844h103.08L408-658.31l-27.77 27.54L207-804h-47v47l174 174-28.31 28.54ZM370-327l430-430v-47h-47L323-374l47 47Zm0 0-23.23-23.77L323-374l23.77 23.23L370-327Z"/></svg>',me=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-121.54q-120.54-35.77-200.27-146.04Q200-377.85 200-516v-216.31l280-104.61 280 104.61V-516q0 138.15-79.73 248.42Q600.54-157.31 480-121.54Zm0-42.46q104-33 172-132t68-220v-189l-240-89.23L240-705v189q0 121 68 220t172 132Zm0-315.23Z"/></svg>',pe=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M200-160q-25.54 0-36.31-22.81-10.77-22.81 5.08-42.57L400-506.15V-760h-55.38q-8.5 0-14.25-5.76t-5.75-14.27q0-8.51 5.75-14.24t14.25-5.73h270.76q8.5 0 14.25 5.76t5.75 14.27q0 8.51-5.75 14.24T615.38-760H560v253.85l231.23 280.77q15.85 19.76 5.08 42.57T760-160H200Zm80-80h400L544-400H416L280-240Zm-80 40h560L520-492v-268h-80v268L200-200Zm280-280Z"/></svg>';class ge extends HTMLElement{constructor(){super(),this._adventurer=null,this._metaState=null}set adventurer(e){this._adventurer=e,this.render()}set metaState(e){this._metaState=e,this.render()}get adventurer(){return this._adventurer}connectedCallback(){this.render()}render(){var s;if(!this._adventurer){this.innerHTML="";return}const e=((s=this._metaState)==null?void 0:s.adventurers)||1,t=this._adventurer.hp/this._adventurer.maxHp*100;this.innerHTML=`
            <div class="bg-brand-surface p-6 pixel-corners shadow-xl">
                    ...
                    <div class="space-y-4">
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${ce()} <span class=" text-lg">${n("global.health")}</span></div>
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${he()} <span class=" text-lg">${n("adventurer_status.interest")}</span></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                    <div class="bg-brand-primary/50 p-3 pixel-corners">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${ue()} <span class="ml-2 ">${n("adventurer_status.weapon")}</span></div>
                    ...
                    <div class="bg-brand-primary/50 p-3 pixel-corners">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${me()} <span class="ml-2 ">${n("adventurer_status.armor")}</span></div>
                    ...
                    <div class="bg-brand-primary/50 p-3 pixel-corners">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${pe()} <span class="ml-2 ">${n("adventurer_status.potions")}</span></div>
        ...
        `}}customElements.define("adventurer-status",ge);class fe extends HTMLElement{constructor(){super(),this.addEventListener("submit",e=>{e.preventDefault();const t=this.querySelector("#enemy-count"),s=this.querySelector("#enemy-power"),a=this.querySelector("#enemy-hp"),r={enemyCount:parseInt(t.value,10)||1,enemyPower:parseInt(s.value,10)||10,enemyHp:parseInt(a.value,10)||20};this.dispatchEvent(new CustomEvent("run-encounter",{bubbles:!0,composed:!0,detail:{encounter:r}}))})}connectedCallback(){this.render()}render(){this.innerHTML=`
        ...
        `}}customElements.define("battle-panel",fe);class be extends HTMLElement{constructor(){super(),this._message=""}static get observedAttributes(){return["message"]}attributeChangedCallback(e,t,s){e==="message"&&(this._message=s,this.render())}connectedCallback(){this.render()}render(){this.innerHTML=`
            <div class="bg-brand-primary/50 p-4 pixel-corners text-center italic text-brand-text-muted border border-brand-primary">
                <p>${this._message}</p>
            </div>
        `}}customElements.define("feedback-panel",be);class xe extends HTMLElement{constructor(){super(),this.state="initial",this.decision=null,this.newlyUnlocked=[],this.addEventListener("click",e=>{const t=e.composedPath()[0];t.id==="unlock-dismiss-button"?this.dismissUnlock():t.id==="continue-run-button"?this.dispatchEvent(new CustomEvent("run-decision",{bubbles:!0,composed:!0,detail:{decision:"continue"}})):t.id==="retire-run-button"&&this.dispatchEvent(new CustomEvent("run-decision",{bubbles:!0,composed:!0,detail:{decision:"retire"}}))})}static get observedAttributes(){return["workshop-unlocked"]}setDecision(e){this.decision=e,this.startFlow()}connectedCallback(){this.render()}startFlow(){this.newlyUnlocked.length>0?this.renderUnlock():(this.state="unlock-revealed",this.revealDecision())}renderUnlock(){const e=this.querySelector("#unlock-container");if(!e)return;const t=Y.find(s=>s.feature===this.newlyUnlocked[0]);t&&(e.innerHTML=`
    ...
    `};class Se extends HTMLElement{constructor(){super(),this._item=null,this._isSelected=!1,this._isDisabled=!1,this._isNewlyDrafted=!1,this.addEventListener("click",()=>{!this._isDisabled&&this._item&&this.dispatchEvent(new CustomEvent("loot-card-select",{bubbles:!0,composed:!0,detail:{instanceId:this._item.instanceId}}))}),this.addEventListener("animationend",e=>{e.animationName==="newly-drafted-animation"&&this.classList.remove("animate-newly-drafted")})}set item(e){this._item=e,this.render()}get item(){return this._item}set isSelected(e){this._isSelected=e,this.render()}get isSelected(){return this._isSelected}set isDisabled(e){this._isDisabled=e,this.render()}get isDisabled(){return this._isDisabled}set isNewlyDrafted(e){this._isNewlyDrafted!==e&&(this._isNewlyDrafted=e,this._isNewlyDrafted&&this.classList.add("animate-newly-drafted"))}get isNewlyDrafted(){return this._isNewlyDrafted}connectedCallback(){this.render()}render(){if(!this._item)return;const e=ye[this._item.rarity]||"text-gray-400",t="bg-brand-surface border pixel-corners p-4 flex flex-col justify-between transition-all duration-200 shadow-lg";let s="";this._isDisabled?s="border-gray-600 opacity-50 cursor-not-allowed":this._isSelected?s="border-brand-secondary scale-105 ring-2 ring-brand-secondary cursor-pointer transform":s="border-brand-primary hover:border-brand-secondary cursor-pointer transform hover:scale-105";const a=this.classList.contains("animate-newly-drafted")?" animate-newly-drafted":"";this.className=`${t} ${s}${a}`,this.innerHTML=`
        ...
        `}}customElements.define("loot-card",Se);const P=4;class $e extends HTMLElement{constructor(){super(),this._choices=[],this._disabled=!1,this._selectedIds=[],this._initialRender=!0,this.addEventListener("loot-card-select",e=>{const{instanceId:t}=e.detail;this.handleSelect(t)}),this.addEventListener("click",e=>{e.target.id==="present-offer-button"&&this.dispatchEvent(new CustomEvent("present-offer",{bubbles:!0,composed:!0,detail:{ids:this._selectedIds}}))})}set choices(e){this._choices=e,this._initialRender=!0,this.render()}get choices(){return this._choices}set disabled(e){this._disabled=e,this.render()}get disabled(){return this._disabled}connectedCallback(){this.render()}handleSelect(e){if(this._disabled)return;const t=new Map(this._choices.map(r=>[r.instanceId,r.id])),s=this._choices.find(r=>r.instanceId===e);if(!s)return;if(this._selectedIds.includes(e))this._selectedIds=this._selectedIds.filter(r=>r!==e);else{if(this._selectedIds.map(o=>t.get(o)).includes(s.id))return;this._selectedIds.length<P&&this._selectedIds.push(e)}this.render()}render(){if(!this._choices)return;const e=new Map(this._choices.map(r=>[r.instanceId,r.id])),t=this._selectedIds.map(r=>e.get(r)),s=this._selectedIds.length>=2&&this._selectedIds.length<=P;this.innerHTML=`
            <div class="w-full">
                ...
                <div class="text-center mt-6">
                    ...
                    >
                        ${n("loot_choice_panel.present_offer")} (${this._selectedIds.length}/${P})
                    </button>
                </div>
            </div>
        `;const a=this.querySelector("#loot-card-container");a&&this._choices.forEach(r=>{const o=document.createElement("loot-card");o.item=r,o.isSelected=this._selectedIds.includes(r.instanceId);const l=!o.isSelected&&t.includes(r.id);o.isDisabled=l||this._disabled,o.isNewlyDrafted=r.justDrafted&&this._initialRender||!1,a.appendChild(o)}),setTimeout(()=>{this._initialRender=!1},0)}}customElements.define("loot-choice-panel",$e);const q=(i,e)=>{const t=e>0;return`
        <div class="flex justify-between text-sm ${t?"text-green-400":"text-red-400"}">
            <span>${i}</span>
            <span class="font-mono">${t?"+":""}${e}</span>
        </div>
    `},ke=(i,e)=>{const s={Common:"text-rarity-common",Uncommon:"text-rarity-uncommon",Rare:"text-rarity-rare"}[i.rarity]||"text-gray-400";return`
        <div class="bg-brand-surface border border-brand-primary rounded-lg p-4 flex flex-col justify-between shadow-lg animate-fade-in">
            <div>
                ...
                <div class="space-y-1 text-brand-text mb-4">
                    ${i.stats.hp?q(n("global.health"),i.stats.hp):""}
                    ${i.stats.maxHp?q(n("global.max_hp"),i.stats.maxHp):""}
                    ${i.stats.power?q(n("global.power"),i.stats.power):""}
                </div>
            </div>
    ...
    `};class Ee extends HTMLElement{constructor(){super(),this._items=[],this._balancePoints=0,this.addEventListener("click",e=>{const t=e.target,s=t.dataset.itemId;s&&this.dispatchEvent(new CustomEvent("purchase-item",{bubbles:!0,composed:!0,detail:{itemId:s}})),t.id==="start-run-button"&&this.dispatchEvent(new CustomEvent("start-run",{bubbles:!0,composed:!0}))})}set items(e){this._items=e,this.render()}set balancePoints(e){this._balancePoints=e,this.render()}connectedCallback(){this.render()}render(){const e=this._items.map(t=>ke(t,this._balancePoints>=(t.cost||0))).join("");this.innerHTML=`
        ...
        `}}customElements.define("menu-screen",Ie);const p=document.getElementById("app");if(!p)throw new Error("Could not find app element to mount to");const J=new K,u=new re(J);u.on("state-change",i=>{if(u.isLoading){p.innerHTML=`<div>${n("global.loading_game_data")}</div>`;return}if(u.error){p.innerHTML=`
            ...
            `;return}le(p,i,u)});p.addEventListener("present-offer",i=>{const{ids:e}=i.detail;u.presentOffer(e)});p.addEventListener("run-encounter",i=>{const{encounter:e}=i.detail;u.runEncounter(e)});p.addEventListener("run-decision",i=>{const{decision:e}=i.detail;u.handleEndOfRun(e)});p.addEventListener("purchase-item",i=>{const{itemId:e}=i.detail;u.purchaseItem(e)});p.addEventListener("start-game",()=>{u.startNewGame()});p.addEventListener("start-run",()=>{u.startNewRun()});p.addEventListener("continue-game",()=>{u.continueGame()});p.addEventListener("reset-game",()=>{J.reset(),u.startNewGame()});async function He(){await ae(),await u.init(),p.innerHTML=`<div>${n("global.initializing")}</div>`,u.showMenu()}He();
