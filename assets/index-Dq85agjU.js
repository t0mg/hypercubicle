(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(a){if(a.ep)return;a.ep=!0;const i=t(a);fetch(a.href,i)}})();const R={hp:100,maxHp:100,power:5};class N{constructor(e,t){this.hp=R.hp,this.maxHp=R.maxHp,this.power=R.power,this.interest=33+Math.floor(Math.random()*50),this.traits=e,this.inventory={weapon:null,armor:null,potions:[]},this.logger=t}modifyInterest(e,t){const s=Math.max(.1,(1e3-this.traits.expertise)/1e3),a=(Math.random()*2-1)*t,i=(e+a)*s,o=this.interest;this.interest=Math.max(0,Math.min(100,this.interest+i)),this.logger.debug(`Interest changed from ${o.toFixed(1)} to ${this.interest.toFixed(1)} (Base: ${e}, Rand: ${a.toFixed(1)}, Total: ${i.toFixed(1)})`)}equip(e){e.type==="Weapon"?this.inventory.weapon=e:e.type==="Armor"&&(this.inventory.armor=e),this.recalculateStats()}addPotion(e){e.type==="Potion"&&this.inventory.potions.push(e)}recalculateStats(){let e=R.power,t=R.maxHp;this.inventory.weapon&&(e+=this.inventory.weapon.stats.power||0,t+=this.inventory.weapon.stats.maxHp||0),this.inventory.armor&&(e+=this.inventory.armor.stats.power||0,t+=this.inventory.armor.stats.maxHp||0);const s=t-this.maxHp;this.power=e,this.maxHp=t,this.hp+=Math.max(0,s)}}class P{constructor(){this.entries=[]}log(e,t="INFO"){this.entries.push({message:e,level:t,timestamp:Date.now()}),console.log(`[${t}] ${e}`)}debug(e){this.log(e,"DEBUG")}info(e){this.log(e,"INFO")}warn(e){this.log(e,"WARN")}error(e){this.log(e,"ERROR")}}const q=15,ee=99,te=10,U=10,T=32,se=8,G=300,F=r=>`${r}_${Math.random().toString(36).substr(2,9)}`,ne=(r,e)=>Math.floor(Math.random()*(e-r+1))+r,z=r=>{const e=[...r];for(let t=e.length-1;t>0;t--){const s=Math.floor(Math.random()*(t+1));[e[t],e[s]]=[e[s],e[t]]}return e},Q=(r,e,t,s)=>{const a=e.filter(d=>r.includes(d.id)),i=[],o={Common:.6,Uncommon:.3,Rare:.1},l={Common:0,Uncommon:0,Rare:0},h={Common:0,Uncommon:0,Rare:0};Object.keys(o).forEach(d=>{h[d]=Math.floor(t*o[d])});let p=Object.values(h).reduce((d,g)=>d+g,0);for(;p<t;)h.Common+=1,p+=1;a.filter(d=>d.cost!==null).forEach(d=>{i.push(s(d)),l[d.rarity]+=1}),Object.keys(o).forEach((d,g)=>{const c=a.filter(m=>m.rarity===d);for(;l[d]<h[d]&&c.length!==0;){const m=Math.floor(Math.random()*c.length),f=c[m];i.push(s(f)),l[d]+=1}});const u=a.filter(d=>d.rarity==="Common");for(;i.length<t&&u.length>0;){const d=Math.floor(Math.random()*u.length),g=u[d];i.push(s(g))}return z(i)},j=(r,e,t)=>Q(r,e,t,s=>({...s,instanceId:F(s.id)})),B=(r,e,t)=>Q(r,e,t,a=>{const i={...a,instanceId:F(a.id)};return i.type==="enemy"&&i.stats.minUnits&&i.stats.maxUnits&&(i.units=ne(i.stats.minUnits,i.stats.maxUnits)),i}),ae=r=>r.roomHand.length<3&&!r.roomHand.some(e=>e.type==="boss"),ie=r=>[...new Set(r.hand.map(t=>t.id))].length<2&&r.hand.length>0;let V={};async function K(r){try{const e=await fetch(`/rogue-steward/locales/${r}.json`);if(!e.ok)throw new Error(`Could not load ${r}.json`);V=await e.json()}catch(e){console.warn(`Failed to load ${r} translations:`,e),r!=="en"&&await K("en")}}function re(){return navigator.language.split("-")[0]}function n(r,e={}){let s=r.split(".").reduce((a,i)=>a?a[i]:void 0,V);if(!s)return console.warn(`Translation not found for key: ${r}`),r;for(const a in e)s=s.replace(`{${a}}`,String(e[a]));return s}async function oe(){const r=re();await K(r)}var D=(r=>(r.WORKSHOP="workshop",r.HAND_SIZE_INCREASE="hand_size_increase",r.CUSTOM_ENCOUNTERS="custom_encounters",r.PARTY_CONSTRAINTS="party_constraints",r.BOSS_FIGHTS="boss_fights",r))(D||{});const Y=[{feature:"workshop",runThreshold:2,title:()=>n("unlocks.workshop.title"),description:()=>n("unlocks.workshop.description")},{feature:"hand_size_increase",runThreshold:3,title:()=>n("unlocks.hand_size_increase.title"),description:()=>n("unlocks.hand_size_increase.description")},{feature:"boss_fights",runThreshold:5,title:()=>n("unlocks.boss_fights.title"),description:()=>n("unlocks.boss_fights.description")},{feature:"custom_encounters",runThreshold:8,title:()=>n("unlocks.custom_encounters.title"),description:()=>n("unlocks.custom_encounters.description")},{feature:"party_constraints",runThreshold:12,title:()=>n("unlocks.party_constraints.title"),description:()=>n("unlocks.party_constraints.description")}];function le(r,e,t){var d,g;const{traits:s,inventory:a}=r;t.debug(`--- Adventurer Decision --- (Offense: ${s.offense}, Risk: ${s.risk})`);const i=((d=a.weapon)==null?void 0:d.stats.power)||0,o=((g=a.armor)==null?void 0:g.stats.maxHp)||0;t.debug(`Current Gear: Weapon Power(${i}), Armor HP(${o})`);const l=c=>{var f,_;let m=(c.rarity==="Uncommon"?2:c.rarity==="Rare"?3:1)*5;switch(c.type){case"Weapon":const b=(c.stats.power||0)-i;if(b<=0&&c.id!==((f=a.weapon)==null?void 0:f.id))return-1;m+=b*(s.offense/10),b>0&&(m+=b*(s.expertise/10));const $=c.stats.maxHp||0;$<0&&(m+=$*(100-s.risk)/20);break;case"Armor":const w=(c.stats.maxHp||0)-o;if(w<=0&&c.id!==((_=a.armor)==null?void 0:_.id))return-1;m+=w*(100-s.offense)/10,w>0&&(m+=w*(s.expertise/10));const I=c.stats.power||0;I>0&&(m+=I*(s.offense/15));const E=c.stats.power||0;E<0&&(m+=E*(s.risk/10));break;case"Potion":const S=r.hp/r.maxHp;m+=10*(100-s.risk)/100,S<.7&&(m+=20*(1-S)),m+=5*(s.expertise/100),a.potions.length>=ee&&(m*=.1);break}return m},h=e.map(c=>({item:c,score:l(c)})).filter(c=>c.score>0);if(h.sort((c,m)=>m.score-c.score),h.length===0||h[0].score<te)return r.modifyInterest(-15,10),{choice:null,reason:n("game_engine.adventurer_declines_offer")};const p=h[0].item;t.debug(`Adventurer chooses: ${p.name} (Score: ${h[0].score.toFixed(1)})`),h[0].score>60?r.modifyInterest(15,5):h[0].score>30?r.modifyInterest(10,8):r.modifyInterest(5,5);const u=n("game_engine.adventurer_accepts_offer",{itemName:p.name});return{choice:p,reason:u}}class ce{constructor(e){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._allRooms=[],this._listeners={},this.init=async()=>{await this._loadGameData()},this.startNewGame=()=>{this.metaManager.incrementAdventurers();const t={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},s=new P,a=new N(t,s),i=this._allItems.filter(m=>m.cost===null).map(m=>m.id),o=j(i,this._allItems,T),l=this._getHandSize(),h=o.slice(0,l),p=o.slice(l),u=this._allRooms.filter(m=>m.cost===null).map(m=>m.id),d=B(u,this._allRooms,T),g=d.slice(0,l),c=d.slice(l);s.info("--- Starting New Game (Run 1) ---"),this.gameState={phase:"DESIGNER_CHOOSING_ROOM",designer:{balancePoints:0},adventurer:a,unlockedDeck:i,availableDeck:p,hand:h,unlockedRoomDeck:u,availableRoomDeck:c,roomHand:g,handSize:l,shopItems:[],offeredLoot:[],offeredRooms:[],feedback:n("game_engine.new_adventurer"),logger:s,run:1,room:1,runEnded:{isOver:!1,reason:""},newlyUnlocked:[]},this._emit("state-change",this.gameState)},this.continueGame=()=>{this.startNewGame()},this.startNewRun=t=>{if(!this.gameState)return;const s=t||this.gameState.run+1;this.metaManager.updateRun(s);const a=this._getHandSize(),i=j(this.gameState.unlockedDeck,this._allItems,T),o=i.slice(0,a),l=i.slice(a),h=B(this.gameState.unlockedRoomDeck,this._allRooms,T),p=h.slice(0,a),u=h.slice(a),d=new N(this.gameState.adventurer.traits,this.gameState.logger);d.interest=this.gameState.adventurer.interest,this.gameState.logger.info(`--- Starting Run ${s} ---`),this.gameState={...this.gameState,adventurer:d,phase:"DESIGNER_CHOOSING_ROOM",availableDeck:l,hand:o,availableRoomDeck:u,roomHand:p,handSize:a,room:1,run:s,feedback:n("game_engine.adventurer_returns"),runEnded:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=t=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(a=>t.includes(a.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;let a=null,i;const o=this.gameState.adventurer;if(this.gameState.offeredLoot.length===0)o.modifyInterest(-15,5),i=n("game_engine.adventurer_declines_empty_offer");else{const f=le(o,this.gameState.offeredLoot,this.gameState.logger);a=f.choice,i=f.reason}let l=this.gameState.hand,h=this.gameState.availableDeck;l.forEach(f=>f.justDrafted=!1);let p=l.filter(f=>!t.includes(f.instanceId));const u=this.gameState.handSize-p.length,d=h.slice(0,u);d.forEach(f=>{f.draftedRoom=this.gameState.room,f.justDrafted=!0});const g=h.slice(u);p.push(...d),a?a.type==="Potion"?o.addPotion(a):o.equip(a):t.length>0&&(o.interest=Math.max(0,o.interest-10));const c=this.gameState.room+1,m=this.gameState.designer.balancePoints+U;this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_ROOM",adventurer:o,feedback:i,availableDeck:g,hand:p,room:c,designer:{balancePoints:m}},this._emit("state-change",this.gameState)},G)},this.runEncounter=t=>{!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_ROOM"||(this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.offeredRooms=t,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.offeredRooms)return;let s=this.gameState.adventurer,a=[];const i=Math.floor(Math.random()*this.gameState.offeredRooms.length),o=this.gameState.offeredRooms[i];switch(this.gameState.logger.info(`--- Encountering Room: ${o.name} ---`),o.type){case"enemy":case"boss":const m={enemyCount:o.units||1,enemyPower:o.stats.attack||5,enemyHp:o.stats.hp||10},f=this._simulateEncounter(s,this.gameState.room,m);s=f.newAdventurer,a=f.feedback;break;case"healing":const _=o.stats.hp||0;s.modifyInterest(s.hp/s.maxHp<.33?20:10,3),s.hp=Math.min(s.maxHp,s.hp+_),a.push(n("game_engine.healing_room",{name:o.name,healing:_})),this.gameState.logger.info(n("game_engine.healing_room",{name:o.name,healing:_}));break;case"trap":const b=o.stats.attack||0;s.hp-=b,s.modifyInterest(s.hp<0?-25:-15,5),a.push(n("game_engine.trap_room",{name:o.name,damage:b})),this.gameState.logger.info(n("game_engine.trap_room",{name:o.name,damage:b}));break}let l=this.gameState.roomHand,h=this.gameState.availableRoomDeck;l.forEach(m=>m.justDrafted=!1);const p=this.gameState.offeredRooms.map(m=>m.instanceId);let u=l.filter(m=>!p.includes(m.instanceId));const d=this.gameState.handSize-u.length,g=h.slice(0,d);g.forEach(m=>{m.draftedRoom=this.gameState.room,m.justDrafted=!0});const c=h.slice(d);if(u.push(...g),this.gameState.adventurer=s,s.hp<=0){this._endRun(n("game_engine.adventurer_fell",{room:this.gameState.room,run:this.gameState.run}));return}if(s.interest<=q){this._endRun(n("game_engine.adventurer_bored",{room:this.gameState.room,run:this.gameState.run}));return}this.gameState.hand&&this.gameState.hand.length===0?(this.gameState.logger.warn("Your hand is empty! The adventurer must press on without new items."),a.push(n("game_engine.empty_hand")),this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_ROOM",room:this.gameState.room+1,designer:{balancePoints:this.gameState.designer.balancePoints+U},feedback:a,encounter:void 0,roomHand:u,availableRoomDeck:c}):this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",feedback:a,encounter:void 0,roomHand:u,availableRoomDeck:c},this._emit("state-change",this.gameState)},G))},this.forceEndRun=()=>{this.gameState&&(this.gameState.adventurer.interest-=30,this._endRun(n("game_engine.no_more_rooms")))},this.enterWorkshop=()=>{if(!this.gameState)return;if(!this.metaManager.acls.has(D.WORKSHOP)){this.startNewRun();return}const t=this.gameState.run+1,s=this._allItems.filter(o=>o.cost!==null).filter(o=>!this.gameState.unlockedDeck.includes(o.id)),a=this._allRooms.filter(o=>o.cost!==null).filter(o=>!this.gameState.unlockedRoomDeck.includes(o.id)),i=[...s,...a];this.gameState={...this.gameState,phase:"SHOP",run:t,room:0,shopItems:z(i).slice(0,4),runEnded:{isOver:!1,reason:""},feedback:n("game_engine.welcome_to_workshop")},this._emit("state-change",this.gameState)},this.purchaseItem=t=>{if(!this.gameState)return;const s=this._allItems.find(u=>u.id===t),a=this._allRooms.find(u=>u.id===t),i=s||a;if(!i||i.cost===null||this.gameState.designer.balancePoints<i.cost)return;let o=this.gameState.unlockedDeck,l=this.gameState.unlockedRoomDeck;s?o=[...this.gameState.unlockedDeck,t]:a&&(l=[...this.gameState.unlockedRoomDeck,t]);const h=this.gameState.designer.balancePoints-i.cost,p=this.gameState.shopItems.filter(u=>u.id!==t);this.gameState.logger.info(`Purchased ${i.name}.`),this.gameState={...this.gameState,designer:{balancePoints:h},unlockedDeck:o,unlockedRoomDeck:l,shopItems:p},this._emit("state-change",this.gameState)},this.showMenu=()=>{this.gameState={...this.gameState||this._getInitialGameState(),phase:"MENU"},this._emit("state-change",this.gameState)},this.metaManager=e}on(e,t){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(t)}_emit(e,t){const s=this._listeners[e];s&&s.forEach(a=>a(t))}_simulateEncounter(e,t,s){var d,g,c,m,f,_,b,$,w,I,E;(d=this.gameState)==null||d.logger.info(`--- Encounter: Room ${t} ---`);const a=[];let i=0,o=0;const l=e.hp;for(let S=0;S<s.enemyCount;S++){(g=this.gameState)==null||g.logger.info(`Adventurer encounters enemy ${S+1}/${s.enemyCount}.`);const J=e.hp/e.maxHp,X=1-e.traits.risk/120;if(J<X&&e.inventory.potions.length>0){const k=e.inventory.potions.shift();if(k){const M=k.stats.hp||0;e.hp=Math.min(e.maxHp,e.hp+M),a.push(n("game_engine.adventurer_drinks_potion",{potionName:k.name})),(c=this.gameState)==null||c.logger.info(`Adventurer used ${k.name} and recovered ${M} HP.`)}}let L=s.enemyHp;for(;L>0&&e.hp>0;){const k=Math.min(.95,.75+e.traits.expertise/500+e.traits.offense/1e3);if(Math.random()<k){const C=e.power;L-=C,(m=this.gameState)==null||m.logger.debug(`Adventurer hits for ${C} damage.`)}else(f=this.gameState)==null||f.logger.debug("Adventurer misses.");if(L<=0){(_=this.gameState)==null||_.logger.info("Enemy defeated."),o++;break}const M=Math.max(.4,.75-e.traits.expertise/500-(100-e.traits.offense)/1e3);if(Math.random()<M){const C=(((b=e.inventory.armor)==null?void 0:b.stats.maxHp)||0)/10,O=Math.max(1,s.enemyPower-C);i+=O,e.hp-=O,($=this.gameState)==null||$.logger.debug(`Enemy hits for ${O} damage.`)}else(w=this.gameState)==null||w.logger.debug("Enemy misses.")}if(e.hp<=0){(I=this.gameState)==null||I.logger.warn("Adventurer has been defeated.");break}}let h;const p=l-e.hp,u=p/e.maxHp;return(E=this.gameState)==null||E.logger.debug(`hpLost: ${p}, hpLostRatio: ${u.toFixed(2)}`),u>.7?(h=n("game_engine.too_close_for_comfort"),e.modifyInterest(-15,5)):u>.4?(h=n("game_engine.great_battle"),e.modifyInterest(10,5)):o>3&&e.traits.offense>60?(h=n("game_engine.easy_fight"),e.modifyInterest(0,5)):(h=n("game_engine.worthy_challenge"),e.modifyInterest(-2,3)),a.push(h),e.hp>0&&o===s.enemyCount&&(e.traits.expertise+=1),{newAdventurer:e,feedback:a,totalDamageTaken:i}}_endRun(e){if(!this.gameState)return;const t=this.metaManager.checkForUnlocks(this.gameState.run);this.gameState.logger.error(`GAME OVER: ${e}`),this.gameState={...this.gameState,phase:"RUN_OVER",runEnded:{isOver:!0,reason:e},newlyUnlocked:t},this._emit("state-change",this.gameState)}getAdventurerEndRunDecision(){if(!this.gameState)return"retire";const{interest:e}=this.gameState.adventurer,t=e-q,s=(Math.random()-.5)*20;return t+s>0?"continue":"retire"}handleEndOfRun(e){if(this.gameState){if(e==="retire"){this.showMenu();return}this.enterWorkshop()}}_getInitialGameState(){const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},t=new P,s=new N(e,t);return{phase:"MENU",designer:{balancePoints:0},adventurer:s,unlockedDeck:[],availableDeck:[],hand:[],unlockedRoomDeck:[],availableRoomDeck:[],roomHand:[],handSize:this._getHandSize(),shopItems:[],offeredLoot:[],offeredRooms:[],feedback:"",logger:t,run:0,room:0,runEnded:{isOver:!1,reason:""},newlyUnlocked:[]}}_getHandSize(){return this.metaManager.acls.has(D.HAND_SIZE_INCREASE)?12:se}isWorkshopUnlocked(){return this.metaManager.acls.has(D.WORKSHOP)}async _loadGameData(){try{const e=await fetch("/rogue-steward/game/items.json");if(!e.ok)throw new Error(n("global.error_loading_items",{statusText:e.statusText}));this._allItems=await e.json();const t=await fetch("/rogue-steward/game/rooms.json");if(!t.ok)throw new Error(n("global.error_loading_rooms",{statusText:t.statusText}));this._allRooms=await t.json()}catch(e){this.error=e.message||n("global.unknown_error"),this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}const de=r=>{if(!r)return n("global.initializing");switch(r.phase){case"AWAITING_ADVENTURER_CHOICE":return n("main.adventurer_considering_offer");case"AWAITING_ENCOUNTER_FEEDBACK":return n("main.adventurer_facing_encounter");default:return n("global.loading")}},he=r=>{const e=document.createElement("loading-indicator");return e.setAttribute("text",de(r)),e},Z=(r,e,t)=>{const s=document.createElement("choice-panel");return s.engine=e,t==="item"?(s.choices=r.hand,s.deckType="item",s.offerImpossible=ie(r)):(s.choices=r.roomHand,s.deckType="room",s.roomSelectionImpossible=ae(r)),s},me=(r,e,t)=>{r.innerHTML="";const s=document.createElement("div");if(s.className="min-h-screen p-4 md:p-6 lg:p-8 flex flex-col items-center",r.appendChild(s),e.runEnded.isOver){const c=document.createElement("run-ended-screen");c.setAttribute("final-bp",e.designer.balancePoints.toString()),c.setAttribute("reason",e.runEnded.reason),c.setAttribute("run",e.run.toString()),t.isWorkshopUnlocked()&&c.setAttribute("workshop-unlocked",""),c.newlyUnlocked=e.newlyUnlocked,c.engine=t,c.setDecision(t.getAdventurerEndRunDecision()),s.appendChild(c)}const a=document.createElement("div");a.className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6",s.appendChild(a);const i=document.createElement("div");i.className="lg:col-span-1 space-y-6",a.appendChild(i);const o=document.createElement("log-panel");o.logger=e.logger,o.traits=e.adventurer.traits,i.appendChild(o);const l=document.createElement("game-stats");t.isWorkshopUnlocked()&&l.setAttribute("balance-points",e.designer.balancePoints.toString()),l.setAttribute("run",e.run.toString()),l.setAttribute("room",e.room.toString()),l.setAttribute("deck-size",e.availableDeck.length.toString()),i.appendChild(l);const h=document.createElement("feedback-panel"),p=Array.isArray(e.feedback)?e.feedback.join(" "):e.feedback;h.setAttribute("message",p),i.appendChild(h);const u=document.createElement("div");u.className="lg:col-span-2 space-y-6",a.appendChild(u);const d=document.createElement("adventurer-status");d.metaState=t.metaManager.metaState,d.adventurer=e.adventurer,u.appendChild(d);const g=document.createElement("div");switch(g.className="lg:col-span-3",a.appendChild(g),e.phase){case"DESIGNER_CHOOSING_LOOT":g.appendChild(Z(e,t,"item"));break;case"DESIGNER_CHOOSING_ROOM":g.appendChild(Z(e,t,"room"));break;case"AWAITING_ADVENTURER_CHOICE":case"AWAITING_ENCOUNTER_FEEDBACK":g.appendChild(he(e));break;default:const c=document.createElement("div");c.textContent=n("main.unhandled_game_phase",{phase:e.phase}),g.appendChild(c);break}},ue=(r,e)=>{r.innerHTML="";const t=document.createElement("menu-screen");t.engine=e,t.metaManager=e.metaManager,r.appendChild(t)},pe=(r,e,t)=>{r.innerHTML="";const s=document.createElement("workshop-screen");s.items=e.shopItems,s.balancePoints=e.designer.balancePoints,s.engine=t,r.appendChild(s)},ge=(r,e,t)=>{if(!e){r.innerHTML=`<div>${n("global.loading")}</div>`;return}switch(e.phase){case"MENU":ue(r,t);break;case"SHOP":pe(r,e,t);break;default:me(r,e,t);break}},W="rogue-steward-meta";class fe{constructor(){this._metaState=this._load()}get metaState(){return this._metaState}get acls(){return new Set(this._metaState.unlockedFeatures)}checkForUnlocks(e){const t=[];for(const s of Y)e>=s.runThreshold&&!this._metaState.unlockedFeatures.includes(s.feature)&&(this._metaState.unlockedFeatures.push(s.feature),t.push(s.feature));return t.length>0&&this.save(),t}incrementAdventurers(){this._metaState.adventurers=(this._metaState.adventurers||0)+1,this.save()}updateRun(e){e>this._metaState.highestRun&&(this._metaState.highestRun=e,this.save())}_load(){try{const e=localStorage.getItem(W);if(e){const t=JSON.parse(e);if(typeof t.highestRun=="number"&&Array.isArray(t.unlockedFeatures))return typeof t.adventurers!="number"&&(t.adventurers=1),t}}catch(e){console.error("Failed to load meta state:",e)}return this._defaultState()}save(){try{localStorage.setItem(W,JSON.stringify(this._metaState))}catch(e){console.error("Failed to save meta state:",e)}}reset(){this._metaState=this._defaultState(),this.save()}_defaultState(){return{highestRun:0,unlockedFeatures:[],adventurers:0}}}const be=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-red-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd" /></svg>',_e=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-blue-400" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M240-400q0 68.92 36.77 126.58 36.77 57.65 98.85 86.5-7.93-11.93-11.77-24.77Q360-224.54 360-238q0-23.54 9.31-44.62 9.31-21.07 26.15-37.92L480-403.85l85.31 83.31q16.84 16.85 25.77 37.92Q600-261.54 600-238q0 13.46-3.85 26.31-3.84 12.84-11.77 24.77 62.08-28.85 98.85-86.5Q720-331.08 720-400q0-50-18.5-94.5T648-574q-20 13-42 19.5t-45 6.5q-62.77 0-108.27-41-45.5-41-51.73-102.54-39 32.23-69 67.73-30 35.5-50.5 72.77-20.5 37.27-31 75.27-10.5 38-10.5 75.77Zm240 52-57 56q-11 11-17 25t-6 29q0 32 23.5 55t56.5 23q33 0 56.5-23t23.5-55q0-16-6-29.5T537-292l-57-56Zm-40-420.46V-708q0 50.92 35.04 85.46Q510.08-588 561-588q18.77 0 36.58-6.35 17.8-6.34 33.65-18.27l17.23-13.53q52.46 38.92 82 98.92T760-400q0 117.08-81.46 198.54T480-120q-117.08 0-198.54-81.46T200-400q0-102.85 64.58-201.15Q329.15-699.46 440-768.46Z"/></svg>',xe=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-amber-400" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M249.23-80H144.62Q117-80 98.5-98.5 80-117 80-144.62v-104.61h40v104.61q0 10.77 6.92 17.7 6.93 6.92 17.7 6.92h104.61v40Zm461.54 0v-40h104.61q10.77 0 17.7-6.92 6.92-6.93 6.92-17.7v-104.61h40v104.61q0 27.62-18.5 46.12Q843-80 815.38-80H710.77ZM480-250.77q-106.92 0-192.12-60.61Q202.69-372 159.23-480q43.46-108 128.65-168.62 85.2-60.61 192.12-60.61t192.12 60.61Q757.31-588 800.77-480q-43.46 108-128.65 168.62-85.2 60.61-192.12 60.61Zm0-40q91.08 0 163.31-49.54T756.85-480q-41.31-90.15-113.54-139.69-72.23-49.54-163.31-49.54t-163.31 49.54Q244.46-570.15 203.15-480q41.31 90.15 113.54 139.69 72.23 49.54 163.31 49.54Zm0-74.61q48 0 81.31-33.31T594.62-480q0-48-33.31-81.31T480-594.62q-48 0-81.31 33.31T365.38-480q0 48 33.31 81.31T480-365.38Zm0-40q-31.15 0-52.88-21.74-21.74-21.73-21.74-52.88 0-31.15 21.74-52.88 21.73-21.74 52.88-21.74 31.15 0 52.88 21.74 21.74 21.73 21.74 52.88 0 31.15-21.74 52.88-21.73 21.74-52.88 21.74ZM80-710.77v-104.61Q80-843 98.5-861.5 117-880 144.62-880h104.61v40H144.62q-10.77 0-17.7 6.92-6.92 6.93-6.92 17.7v104.61H80Zm760 0v-104.61q0-10.77-6.92-17.7-6.93-6.92-17.7-6.92H710.77v-40h104.61q27.62 0 46.12 18.5Q880-843 880-815.38v104.61h-40ZM480-480Z"/></svg>',ve=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M753.54-132.15 631.15-254.31l-88 88-11.07-11.07q-18.39-18.39-18.39-45.47 0-27.07 18.39-45.46l163.61-163.61q18.39-18.39 45.46-18.39 27.08 0 45.47 18.39l11.07 11.07-88 88 122.16 122.39q9.69 9.69 9.69 22.61 0 12.93-9.69 22.62l-33.08 33.08q-9.69 9.69-22.62 9.69-12.92 0-22.61-9.69ZM840-740.92 398.31-298.46l29.61 29.38q18.39 18.39 18.39 45.46 0 27.08-18.39 45.47l-11.07 11.07-88-88-122.39 122.16q-9.69 9.69-22.61 9.69-12.93 0-22.62-9.69L128.15-166q-9.69-9.69-9.69-22.62 0-12.92 9.69-22.61l122.16-122.39-88-88 11.07-11.07q18.39-18.39 45.47-18.39 27.07 0 45.46 18.39l30.15 30.38L736.92-844H840v103.08ZM334-583l23.23-23.77 23-24-23 24L334-583Zm-28.31 28.54L120-740.92V-844h103.08L408-658.31l-27.77 27.54L207-804h-47v47l174 174-28.31 28.54ZM370-327l430-430v-47h-47L323-374l47 47Zm0 0-23.23-23.77L323-374l23.77 23.23L370-327Z"/></svg>',we=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-121.54q-120.54-35.77-200.27-146.04Q200-377.85 200-516v-216.31l280-104.61 280 104.61V-516q0 138.15-79.73 248.42Q600.54-157.31 480-121.54Zm0-42.46q104-33 172-132t68-220v-189l-240-89.23L240-705v189q0 121 68 220t172 132Zm0-315.23Z"/></svg>',ye=()=>'<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M200-160q-25.54 0-36.31-22.81-10.77-22.81 5.08-42.57L400-506.15V-760h-55.38q-8.5 0-14.25-5.76t-5.75-14.27q0-8.51 5.75-14.24t14.25-5.73h270.76q8.5 0 14.25 5.76t5.75 14.27q0 8.51-5.75 14.24T615.38-760H560v253.85l231.23 280.77q15.85 19.76 5.08 42.57T760-160H200Zm80-80h400L544-400H416L280-240Zm-80 40h560L520-492v-268h-80v268L200-200Zm280-280Z"/></svg>';class Se extends HTMLElement{constructor(){super(),this._adventurer=null,this._metaState=null}set adventurer(e){this._adventurer=e,this.render()}set metaState(e){this._metaState=e,this.render()}get adventurer(){return this._adventurer}connectedCallback(){this.render()}render(){var s;if(!this._adventurer){this.innerHTML="";return}const e=((s=this._metaState)==null?void 0:s.adventurers)||1,t=this._adventurer.hp/this._adventurer.maxHp*100;this.innerHTML=`
            <div class="bg-brand-surface p-6 pixel-corners shadow-xl">
                    ...
                    <div class="space-y-4">
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${be()} <span class=" text-lg">${n("global.health")}</span></div>
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${xe()} <span class=" text-lg">${n("adventurer_status.interest")}</span></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                    <div class="bg-brand-primary/50 p-3 pixel-corners">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${ve()} <span class="ml-2 ">${n("adventurer_status.weapon")}</span></div>
                    ...
                    <div class="bg-brand-primary/50 p-3 pixel-corners">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${we()} <span class="ml-2 ">${n("adventurer_status.armor")}</span></div>
                    ...
                    <div class="bg-brand-primary/50 p-3 pixel-corners">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${ye()} <span class="ml-2 ">${n("adventurer_status.potions")}</span></div>
        ...
        `}}customElements.define("adventurer-status",Se);class ke extends HTMLElement{constructor(){super(),this.addEventListener("submit",e=>{e.preventDefault();const t=this.querySelector("#enemy-count"),s=this.querySelector("#enemy-power"),a=this.querySelector("#enemy-hp"),i={enemyCount:parseInt(t.value,10)||1,enemyPower:parseInt(s.value,10)||10,enemyHp:parseInt(a.value,10)||20};this.dispatchEvent(new CustomEvent("run-encounter",{bubbles:!0,composed:!0,detail:{encounter:i}}))})}connectedCallback(){this.render()}render(){this.innerHTML=`
        ...
        `}}customElements.define("battle-panel",ke);class $e extends HTMLElement{constructor(){super(),this._message=""}static get observedAttributes(){return["message"]}attributeChangedCallback(e,t,s){e==="message"&&(this._message=s,this.render())}connectedCallback(){this.render()}render(){this.innerHTML=`
        ...
        `}}customElements.define("game-stats",Ee);class Re extends HTMLElement{constructor(){super(),this._text="Loading..."}static get observedAttributes(){return["text"]}attributeChangedCallback(e,t,s){e==="text"&&(this._text=s,this.render())}connectedCallback(){this.render()}render(){this.innerHTML=`
    ...
    `}}customElements.define("choice-card",Ce);const A=4;class Te extends HTMLElement{constructor(){super(),this._choices=[],this._deckType="item",this.engine=null,this._disabled=!1,this._selectedIds=[],this._initialRender=!0,this._offerImpossible=!1,this._roomSelectionImpossible=!1,this.addEventListener("card-select",e=>{const{instanceId:t}=e.detail;this.handleSelect(t)}),this.addEventListener("click",e=>{if(e.target.id==="present-offer-button"&&this.engine)if(this._deckType==="item")this.engine.presentOffer(this._selectedIds);else if(this._roomSelectionImpossible)this.engine.forceEndRun();else{const s=this._choices.filter(a=>this._selectedIds.includes(a.instanceId));this.engine.runEncounter(s)}})}set choices(e){this._choices=e,this._initialRender=!0,this.render()}get choices(){return this._choices}set deckType(e){this._deckType=e,this.render()}get deckType(){return this._deckType}set disabled(e){this._disabled=e,this.render()}get disabled(){return this._disabled}set offerImpossible(e){this._offerImpossible=e,this.render()}get offerImpossible(){return this._offerImpossible}set roomSelectionImpossible(e){this._roomSelectionImpossible=e,this.render()}get roomSelectionImpossible(){return this._roomSelectionImpossible}connectedCallback(){this.render()}handleSelect(e){if(this._disabled)return;const t=this._choices.find(a=>a.instanceId===e);if(!t)return;const s=this._selectedIds.includes(e);if(this._deckType==="room"){const a=t.type==="boss";if(s)this._selectedIds=this._selectedIds.filter(i=>i!==e);else{const o=this._choices.filter(l=>this._selectedIds.includes(l.instanceId)).some(l=>l.type==="boss");a&&this._selectedIds.length===0?this._selectedIds.push(e):!a&&!o&&this._selectedIds.length<3&&this._selectedIds.push(e)}}else{const a=new Map(this._choices.map(i=>[i.instanceId,i.id]));if(s)this._selectedIds=this._selectedIds.filter(i=>i!==e);else{if(this._selectedIds.map(o=>a.get(o)).includes(t.id))return;this._selectedIds.length<A&&this._selectedIds.push(e)}}this.render()}render(){if(!this._choices)return;const e=this._deckType==="room",t=n(e?"choice_panel.title_room":"choice_panel.title");let s=n(e?"choice_panel.begin_encounter":"choice_panel.present_offer");this._offerImpossible?s=n("choice_panel.continue_without_loot"):this._roomSelectionImpossible&&(s=n("choice_panel.roll_credits"));let a=!1,i=s;this._offerImpossible||this._roomSelectionImpossible?a=!0:e?this._choices.filter(p=>this._selectedIds.includes(p.instanceId)).some(p=>p.type==="boss")?(a=this._selectedIds.length===1,i=`${s} (1/1)`):(a=this._selectedIds.length===3,i=`${s} (${this._selectedIds.length}/3)`):(a=this._selectedIds.length>=2&&this._selectedIds.length<=A,i=`${s} (${this._selectedIds.length}/${A})`),this.innerHTML=`
        ...
        `;const o=this.querySelector("#loot-card-container");o&&this._choices.forEach(l=>{const h=document.createElement("choice-card");h.item=l,h.isSelected=this._selectedIds.includes(l.instanceId);let p=this._disabled;if(this._offerImpossible)p=!0;else if(e){const u=this._choices.filter(g=>this._selectedIds.includes(g.instanceId)),d=u.some(g=>g.type==="boss");h.isSelected?p=!1:(d||l.type==="boss"&&u.length>0||u.length>=3)&&(p=!0)}else{const u=new Map(this._choices.map(c=>[c.instanceId,c.id])),d=this._selectedIds.map(c=>u.get(c));p=!h.isSelected&&d.includes(l.id)||this._disabled}h.isDisabled=p,h.isNewlyDrafted=l.justDrafted&&this._initialRender||!1,o.appendChild(h)}),setTimeout(()=>{this._initialRender=!1},0)}}customElements.define("choice-panel",Te);const v=(r,e,t=!0)=>`
        <div class="flex justify-between text-sm ${t?"text-green-400":"text-red-400"}">
            <span>${r}</span>
            <span class="font-mono">${t?"+":""}${e}</span>
        </div>
    ...
    `,Le=(r,e)=>{const s={Common:"text-rarity-common",Uncommon:"text-rarity-uncommon",Rare:"text-rarity-rare"}[r.rarity]||"text-gray-400";let a="";if("stats"in r)if(r.type==="Weapon"||r.type==="Armor"||r.type==="Potion"){const i=r;a=`
            ...
            `}else{const i=r;switch(i.type){case"enemy":a=`
                        ...
                        ${i.stats.minUnits&&i.stats.maxUnits?De(n("global.units"),i.stats.minUnits,i.stats.maxUnits):""}
                    `;break;case"boss":a=`
                        ${i.stats.attack?v(n("global.attack"),i.stats.attack,!1):""}
                        ${i.stats.hp?v(n("global.health"),i.stats.hp,!1):""}
    ...
    `};class Oe extends HTMLElement{constructor(){super(),this._items=[],this._balancePoints=0,this.engine=null,this.addEventListener("click",e=>{const t=e.target,s=t.dataset.itemId;s&&this.engine&&this.engine.purchaseItem(s),t.id==="start-run-button"&&this.engine&&this.engine.startNewRun()})}set items(e){this._items=e,this.render()}set balancePoints(e){this._balancePoints=e,this.render()}connectedCallback(){this.render()}render(){const e=this._items.map(t=>Le(t,this._balancePoints>=(t.cost||0))).join("");this.innerHTML=`
        ...
        `}}customElements.define("menu-screen",Ne);const H=document.getElementById("app");if(!H)throw new Error("Could not find app element to mount to");const Ae=new fe,y=new ce(Ae);y.on("state-change",r=>{if(y.isLoading){H.innerHTML=`<div>${n("global.loading_game_data")}</div>`;return}if(y.error){H.innerHTML=`
                <div class="min-h-screen flex items-center justify-center p-4">
                    <div class="bg-brand-surface p-8 rounded-xl shadow-2xl text-center border border-brand-secondary">
                         ...
                         <p class="text-brand-text">${y.error}</p>
                    </div>
                </div>
            `;return}ge(H,r,y)});async function Pe(){await oe(),await y.init(),H.innerHTML=`<div>${n("global.initializing")}</div>`,y.showMenu()}Pe();
