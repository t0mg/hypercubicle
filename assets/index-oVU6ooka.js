(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))t(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&t(o)}).observe(document,{childList:!0,subtree:!0});function s(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function t(i){if(i.ep)return;i.ep=!0;const a=s(i);fetch(i.href,a)}})();let N={};async function q(r){try{const e=await fetch(`/locales/${r}.json`);if(!e.ok)throw new Error(`Could not load ${r}.json`);N=await e.json()}catch(e){console.error("Failed to load translations:",e),r!=="en"&&await q("en")}}function R(){return navigator.language.split("-")[0]}function n(r,e={}){let t=r.split(".").reduce((i,a)=>i?i[a]:null,N);if(!t)return console.warn(`Translation not found for key: ${r}`),r;for(const i in e)t=t.replace(`{${i}}`,String(e[i]));return t}async function P(){const r=R();await q(r)}const H=15,G=3,B={Common:1,Uncommon:2,Rare:3},E={hp:100,maxHp:100,power:5},F=10,I=10,U=["loot_1","loot_2","loot_3","loot_4","loot_5"],Z=32,w=9,M=300,A=r=>[...r].sort(()=>Math.random()-.5),O=(r,e)=>{let s=0;const t=new Map(e.map(l=>[l.id,l])),i=r.map(l=>t.get(l)).filter(Boolean),a=l=>(s++,{...l,instanceId:`${l.id}-${s}`}),o=i.filter(l=>l.cost===null),p=i.filter(l=>l.cost!==null);let g=[];if(o.length>0)for(const l of o)g.push(a(l),a(l),a(l),a(l));g.push(...p.map(a));let u=0;for(;g.length<Z&&o.length>0;)g.push(a(o[u%o.length])),u++;return A(g)};class j{constructor(){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._listeners={},this.startNewGame=()=>{const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},s={...E,interest:33+Math.floor(Math.random()*50),traits:e,inventory:{weapon:null,armor:null,potions:[]}},t=U,i=O(t,this._allItems),a=i.slice(0,w),o=i.slice(w);this.gameState&&this.gameState.log&&console.log("previous game log dump:",this.gameState.log),this.gameState={phase:"DESIGNER_CHOOSING_DIFFICULTY",designer:{balancePoints:0},adventurer:s,unlockedDeck:t,availableDeck:o,hand:a,shopItems:[],offeredLoot:[],feedback:n("game_engine.new_adventurer"),log:["--- Starting New Game (Run 1) ---"],run:1,floor:1,gameOver:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.startNewRun=()=>{if(!this.gameState)return;const e=O(this.gameState.unlockedDeck,this._allItems),s=e.slice(0,w),t=e.slice(w),i={...E,interest:this.gameState.adventurer.interest,traits:this.gameState.adventurer.traits,inventory:{weapon:null,armor:null,potions:[]}};this.gameState={...this.gameState,adventurer:i,phase:"DESIGNER_CHOOSING_DIFFICULTY",availableDeck:t,hand:s,floor:1,feedback:n("game_engine.adventurer_returns"),log:[...this.gameState.log,`--- Starting Run ${this.gameState.run} ---`],gameOver:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=e=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(t=>e.includes(t.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;const{choice:t,reason:i,logs:a}=this._getAdventurerChoice(this.gameState.adventurer,this.gameState.offeredLoot);let o={...this.gameState.adventurer},p=o.interest,g=this.gameState.hand,u=this.gameState.availableDeck;g.forEach(h=>h.justDrafted=!1);let l=g.filter(h=>!e.includes(h.instanceId));const b=w-l.length,_=u.slice(0,b);_.forEach(h=>{h.draftedFloor=this.gameState.floor,h.justDrafted=!0});const v=u.slice(b);if(l.push(..._),t){let h={...o.inventory,potions:[...o.inventory.potions]};t.type==="Weapon"?h.weapon=t:t.type==="Armor"?h.armor=t:t.type==="Potion"&&h.potions.push(t);const{power:y,maxHp:x}=this._recalculateStats(h),S=x-o.maxHp;o.inventory=h,o.power=y,o.maxHp=x,o.hp+=Math.max(0,S)}else p=Math.max(0,this.gameState.adventurer.interest-10);const d=this.gameState.floor+1,c=this.gameState.designer.balancePoints+I;this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:{...o,interest:p},feedback:i,availableDeck:v,hand:l,log:[...this.gameState.log,...a],floor:d,designer:{balancePoints:c}},this._emit("state-change",this.gameState)},M)},this.runDebugEncounter=e=>{!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_DIFFICULTY"||(this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.debugEncounterParams=e,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.debugEncounterParams)return;const{newAdventurer:s,feedback:t,logs:i}=this._getDebugEncounterOutcome(this.gameState.adventurer,this.gameState.floor,this.gameState.debugEncounterParams),a=[...this.gameState.log,...i];if(s.hp<=0){a.push("GAME OVER: Adventurer has fallen in battle."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:this.gameState.designer.balancePoints+I},phase:"RUN_OVER",gameOver:{isOver:!0,reason:n("game_engine.adventurer_fell",{floor:this.gameState.floor,run:this.gameState.run})},log:a},this._emit("state-change",this.gameState);return}if(s.interest<=H){a.push("GAME OVER: Adventurer lost interest and quit."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:this.gameState.designer.balancePoints+I},phase:"RUN_OVER",gameOver:{isOver:!0,reason:n("game_engine.adventurer_bored",{floor:this.gameState.floor,run:this.gameState.run})},log:a},this._emit("state-change",this.gameState);return}let o=t;this.gameState.hand&&this.gameState.hand.length===0?(a.push("Your hand is empty! The adventurer must press on without new items."),o=n("game_engine.empty_hand"),this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:s,floor:this.gameState.floor+1,designer:{balancePoints:this.gameState.designer.balancePoints+I},feedback:o,log:a,debugEncounterParams:void 0}):this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",adventurer:s,feedback:o,log:a,debugEncounterParams:void 0},this._emit("state-change",this.gameState)},M))},this.enterWorkshop=()=>{if(!this.gameState)return;const e=this.gameState.run+1,s=this._allItems.filter(t=>t.cost!==null).filter(t=>!this.gameState.unlockedDeck.includes(t.id)).filter(t=>e>=t.minRun);this.gameState={...this.gameState,phase:"SHOP",run:e,floor:0,shopItems:A(s).slice(0,4),gameOver:{isOver:!1,reason:""},feedback:n("game_engine.welcome_to_workshop")},this._emit("state-change",this.gameState)},this.purchaseItem=e=>{if(!this.gameState)return;const s=this._allItems.find(o=>o.id===e);if(!s||s.cost===null||this.gameState.designer.balancePoints<s.cost)return;const t=[...this.gameState.unlockedDeck,e],i=this.gameState.designer.balancePoints-s.cost,a=this.gameState.shopItems.filter(o=>o.id!==e);this.gameState={...this.gameState,designer:{balancePoints:i},unlockedDeck:t,shopItems:a,log:[...this.gameState.log,`Purchased ${s.name}.`]},this._emit("state-change",this.gameState)},this._loadGameData()}on(e,s){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(s)}_emit(e,s){const t=this._listeners[e];t&&t.forEach(i=>i(s))}_recalculateStats(e){let s=E.power,t=E.maxHp;return e.weapon&&(s+=e.weapon.stats.power||0,t+=e.weapon.stats.maxHp||0),e.armor&&(s+=e.armor.stats.power||0,t+=e.armor.stats.maxHp||0),{power:s,maxHp:t}}_getAdventurerChoice(e,s){var _,v;const{traits:t,inventory:i}=e,a=[`--- Adventurer Decision --- (Offense: ${t.offense}, Risk: ${t.risk})`],o=((_=i.weapon)==null?void 0:_.stats.power)||0,p=((v=i.armor)==null?void 0:v.stats.maxHp)||0;a.push(`Current Gear: Weapon Power(${o}), Armor HP(${p})`);const g=d=>{var h,y;let c=(B[d.rarity]||1)*5;switch(d.type){case"Weapon":const x=(d.stats.power||0)-o;if(x<=0&&d.id!==((h=i.weapon)==null?void 0:h.id))return-1;c+=x*(t.offense/10),x>0&&(c+=x*(t.expertise/10));const S=d.stats.maxHp||0;S<0&&(c+=S*(100-t.risk)/20);break;case"Armor":const $=(d.stats.maxHp||0)-p;if($<=0&&d.id!==((y=i.armor)==null?void 0:y.id))return-1;c+=$*(100-t.offense)/10,$>0&&(c+=$*(t.expertise/10));const D=d.stats.power||0;D>0&&(c+=D*(t.offense/15));const C=d.stats.power||0;C<0&&(c+=C*(t.risk/10));break;case"Potion":const T=e.hp/e.maxHp;c+=10*(100-t.risk)/100,T<.7&&(c+=20*(1-T)),c+=5*(t.expertise/100),i.potions.length>=G&&(c*=.1);break}return c+Math.random()},u=s.map(d=>({item:d,score:g(d)})).filter(d=>d.score>0);if(u.sort((d,c)=>c.score-d.score),u.length===0||u[0].score<F)return{choice:null,reason:n("game_engine.adventurer_declines_offer"),logs:a};const l=u[0].item;let b=n("game_engine.adventurer_accepts_offer",{itemName:l.name});return{choice:l,reason:b,logs:a}}_getDebugEncounterOutcome(e,s,t){const i=[`--- DEBUG Encounter: Floor ${s} ---`];let a=JSON.parse(JSON.stringify(e)),o="";const p=a.hp/a.maxHp,g=1-a.traits.risk/120;if(p<g&&a.inventory.potions.length>0){const d=a.inventory.potions.shift(),c=d.stats.hp||0;a.hp=Math.min(a.maxHp,a.hp+c),o=n("game_engine.adventurer_drinks_potion",{potionName:d.name})}let u=Math.round(t.baseDamage*t.difficultyFactor);u=Math.max(0,u),a.hp-=u;let l=0,b="";const _=u/a.maxHp;let v="Normal";return t.difficultyFactor<=.8?v="Easy":t.difficultyFactor>=1.3&&(v="Hard"),v==="Easy"?(l=-5-Math.floor((100-a.traits.risk)/20),b=n("game_engine.easy_fight")):_<.1?(l=-2,b=n("game_engine.worthy_challenge")):_<.35?(l=5+Math.floor(a.traits.offense/20)+(v==="Hard"?5:0),b=n("game_engine.great_battle")):(l=-10-Math.floor((100-a.traits.risk)/10),b=n("game_engine.too_close_for_comfort")),a.interest=Math.max(0,Math.min(100,a.interest+l)),a.traits.expertise+=1,{newAdventurer:a,feedback:o+b,damageTaken:u,logs:i}}getAdventurerEndRunDecision(){if(!this.gameState)return"retire";const{interest:e}=this.gameState.adventurer,s=e-H,t=(Math.random()-.5)*20;return s+t>0?"continue":"retire"}async _loadGameData(){try{const e=await fetch("/game/items.json");if(!e.ok)throw new Error(n("global.error_loading_items",{statusText:e.statusText}));this._allItems=await e.json(),this.startNewGame()}catch(e){this.error=e.message||n("global.unknown_error"),this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}class W extends HTMLElement{constructor(){super(),this._balancePoints=0,this._run=0,this._floor=0}static get observedAttributes(){return["balance-points","run","floor"]}attributeChangedCallback(e,s,t){switch(e){case"balance-points":this._balancePoints=Number(t);break;case"run":this._run=Number(t);break;case"floor":this._floor=Number(t);break}this.render()}connectedCallback(){this.render()}render(){this.innerHTML=`
            ...
            <div class="bg-brand-surface p-6 rounded-lg shadow-xl border border-gray-700">
                    ...
                    <div class="space-y-4">
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center">${Q()} <span class="font-semibold text-lg">${n("global.health")}</span></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                    <div class="bg-brand-primary/50 p-3 rounded-lg">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${z()} <span class="ml-2 font-semibold">${n("adventurer_status.weapon")}</span></div>
                    ...
                    <div class="bg-brand-primary/50 p-3 rounded-lg">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${J()} <span class="ml-2 font-semibold">${n("adventurer_status.armor")}</span></div>
                    ...
                    <div class="bg-brand-primary/50 p-3 rounded-lg">
                        <div class="flex items-center justify-center text-brand-text-muted mb-1">${X()} <span class="ml-2 font-semibold">${n("adventurer_status.potions")}</span></div>
    ...
    `};class se extends HTMLElement{constructor(){super(),this._item=null,this._isSelected=!1,this._isDisabled=!1,this._isNewlyDrafted=!1,this.addEventListener("click",()=>{!this._isDisabled&&this._item&&this.dispatchEvent(new CustomEvent("loot-card-select",{bubbles:!0,composed:!0,detail:{instanceId:this._item.instanceId}}))}),this.addEventListener("animationend",e=>{e.animationName==="newly-drafted-animation"&&this.classList.remove("animate-newly-drafted")})}set item(e){this._item=e,this.render()}get item(){return this._item}set isSelected(e){this._isSelected=e,this.render()}get isSelected(){return this._isSelected}set isDisabled(e){this._isDisabled=e,this.render()}get isDisabled(){return this._isDisabled}set isNewlyDrafted(e){this._isNewlyDrafted!==e&&(this._isNewlyDrafted=e,this._isNewlyDrafted&&this.classList.add("animate-newly-drafted"))}get isNewlyDrafted(){return this._isNewlyDrafted}connectedCallback(){this.render()}render(){if(!this._item)return;const e=te[this._item.rarity]||"text-gray-400",s="bg-brand-surface border rounded-lg p-4 flex flex-col justify-between transition-all duration-200 shadow-lg";let t="";this._isDisabled?t="border-gray-600 opacity-50 cursor-not-allowed":this._isSelected?t="border-brand-secondary scale-105 ring-2 ring-brand-secondary cursor-pointer transform":t="border-brand-primary hover:border-brand-secondary cursor-pointer transform hover:scale-105";const i=this.classList.contains("animate-newly-drafted")?" animate-newly-drafted":"";this.className=`${s} ${t}${i}`,this.innerHTML=`
        ...
        `}}customElements.define("loot-card",se);class ae extends HTMLElement{constructor(){super(),this._choices=[],this._disabled=!1,this._selectedIds=[],this._initialRender=!0,this.addEventListener("loot-card-select",e=>{const{instanceId:s}=e.detail;this.handleSelect(s)}),this.addEventListener("click",e=>{e.target.id==="present-offer-button"&&this.dispatchEvent(new CustomEvent("present-offer",{bubbles:!0,composed:!0,detail:{ids:this._selectedIds}}))})}set choices(e){this._choices=e,this._initialRender=!0,this.render()}get choices(){return this._choices}set disabled(e){this._disabled=e,this.render()}get disabled(){return this._disabled}connectedCallback(){this.render()}handleSelect(e){if(this._disabled)return;const s=new Map(this._choices.map(a=>[a.instanceId,a.id])),t=this._choices.find(a=>a.instanceId===e);if(!t)return;if(this._selectedIds.includes(e))this._selectedIds=this._selectedIds.filter(a=>a!==e);else{if(this._selectedIds.map(o=>s.get(o)).includes(t.id))return;this._selectedIds.length<3&&this._selectedIds.push(e)}this.render()}render(){if(!this._choices)return;const e=new Map(this._choices.map(a=>[a.instanceId,a.id])),s=this._selectedIds.map(a=>e.get(a)),t=this._selectedIds.length>=2&&this._selectedIds.length<=3;this.innerHTML=`
            <div class="w-full">
                ...
                <div class="text-center mt-6">
                    ...
                    >
                        ${n("loot_choice_panel.present_offer")} (${this._selectedIds.length}/3)
                    </button>
                </div>
            </div>
        `;const i=this.querySelector("#loot-card-container");i&&this._choices.forEach(a=>{const o=document.createElement("loot-card");o.item=a,o.isSelected=this._selectedIds.includes(a.instanceId);const p=!o.isSelected&&s.includes(a.id);o.isDisabled=p||this._disabled,o.isNewlyDrafted=a.justDrafted&&this._initialRender||!1,i.appendChild(o)}),setTimeout(()=>{this._initialRender=!1},0)}}customElements.define("loot-choice-panel",ae);class ne extends HTMLElement{constructor(){super(),this._text="Loading..."}static get observedAttributes(){return["text"]}attributeChangedCallback(e,s,t){e==="text"&&(this._text=t,this.render())}connectedCallback(){this.render()}render(){this.innerHTML=`
        ...
        `}}customElements.define("loading-indicator",ne);class re extends HTMLElement{constructor(){super(),this._defaultBaseDamage=10,this.addEventListener("submit",e=>{e.preventDefault();const s=this.querySelector("#base-damage"),t=this.querySelector("#difficulty-factor"),i={baseDamage:parseFloat(s.value)||0,difficultyFactor:parseFloat(t.value)||0};this.dispatchEvent(new CustomEvent("run-encounter",{bubbles:!0,composed:!0,detail:{params:i}}))})}set defaultBaseDamage(e){this._defaultBaseDamage=e;const s=this.querySelector("#base-damage");s&&(s.value=this._defaultBaseDamage.toString())}connectedCallback(){this.render(),this.defaultBaseDamage=this._defaultBaseDamage}render(){this.innerHTML=`
        ...
        `}}customElements.define("debug-encounter-panel",re);class ie extends HTMLElement{constructor(){super(),this._logs=[],this._traits=null}set logs(e){this._logs=e,this.render()}set traits(e){this._traits=e,this.render()}connectedCallback(){this.render()}render(){if(!this._traits){this.innerHTML="";return}const e=this._logs.map((t,i)=>`<p class="whitespace-pre-wrap">[${i.toString().padStart(3,"0")}] ${t}</p>`).join("");this.innerHTML=`
        ...
        `}updateDecision(e){const s=this.querySelector("#decision-container"),t=this.querySelector("#button-container");if(!s||!t)return;let i="",a="";const o=e?"animate-fade-in-up":"";this.decision==="continue"?(i=`
            ...
            `),s.innerHTML=i,t.innerHTML=a}}customElements.define("game-over-screen",oe);const L=(r,e)=>{const s=e>0;return`
        <div class="flex justify-between text-sm ${s?"text-green-400":"text-red-400"}">
            <span>${r}</span>
            <span class="font-mono">${s?"+":""}${e}</span>
        </div>
    `},le=(r,e)=>{const t={Common:"text-rarity-common",Uncommon:"text-rarity-uncommon",Rare:"text-rarity-rare"}[r.rarity]||"text-gray-400";return`
        <div class="bg-brand-surface border border-brand-primary rounded-lg p-4 flex flex-col justify-between shadow-lg animate-fade-in">
            <div>
                ...
                <div class="space-y-1 text-brand-text mb-4">
                    ${r.stats.hp?L(n("global.health"),r.stats.hp):""}
                    ${r.stats.maxHp?L(n("global.max_hp"),r.stats.maxHp):""}
                    ${r.stats.power?L(n("global.power"),r.stats.power):""}
                </div>
            </div>
    ...
    `};class de extends HTMLElement{constructor(){super(),this._items=[],this._balancePoints=0,this.addEventListener("click",e=>{const s=e.target,t=s.dataset.itemId;t&&this.dispatchEvent(new CustomEvent("purchase-item",{bubbles:!0,composed:!0,detail:{itemId:t}})),s.id==="start-run-button"&&this.dispatchEvent(new CustomEvent("start-run",{bubbles:!0,composed:!0}))})}set items(e){this._items=e,this.render()}set balancePoints(e){this._balancePoints=e,this.render()}connectedCallback(){this.render()}render(){const e=this._items.map(s=>le(s,this._balancePoints>=(s.cost||0))).join("");this.innerHTML=`
        ...
        `}}customElements.define("workshop-screen",de);const m=document.getElementById("app");if(!m)throw new Error("Could not find app element to mount to");const f=new j,ce=r=>{if(!r)return n("global.initializing");switch(r.phase){case"AWAITING_ADVENTURER_CHOICE":return n("main.adventurer_considering_offer");case"AWAITING_ENCOUNTER_FEEDBACK":return n("main.adventurer_facing_encounter");default:return n("global.loading")}},ue=r=>{switch(r.phase){case"DESIGNER_CHOOSING_LOOT":return'<div class="lg:col-span-3"><loot-choice-panel></loot-choice-panel></div>';case"DESIGNER_CHOOSING_DIFFICULTY":return'<div class="lg:col-span-3"><debug-encounter-panel></debug-encounter-panel></div>';case"AWAITING_ADVENTURER_CHOICE":case"AWAITING_ENCOUNTER_FEEDBACK":return`<div class="lg:col-span-3"><loading-indicator text="${ce(r)}"></loading-indicator></div>`;default:return`<div class="lg:col-span-3"><div>${n("main.unhandled_game_phase",{phase:r.phase})}</div></div>`}},he=r=>{if(!r){m.innerHTML=`<div>${n("global.loading")}</div>`;return}if(r.phase==="SHOP"){m.innerHTML="<workshop-screen></workshop-screen>";const o=document.querySelector("workshop-screen");o&&(o.items=r.shopItems,o.balancePoints=r.designer.balancePoints);return}const e=r.gameOver.isOver?`<game-over-screen
                final-bp="${r.designer.balancePoints}"
                reason="${r.gameOver.reason}"
                run="${r.run}"
                decision="${f.getAdventurerEndRunDecision()}"
    ...
    `;const s=document.querySelector("adventurer-status");s&&(s.adventurer=r.adventurer);const t=document.querySelector("loot-choice-panel");t&&(t.choices=r.hand,t.disabled=!1);const i=document.querySelector("debug-encounter-panel");if(i){const o=Math.max(1,15-Math.floor(r.adventurer.power/4)+Math.floor(r.floor*1.5));i.defaultBaseDamage=o}const a=document.querySelector("debug-log");a&&(a.logs=r.log,a.traits=r.adventurer.traits)};m.addEventListener("present-offer",r=>{const{ids:e}=r.detail;f.presentOffer(e)});m.addEventListener("run-encounter",r=>{const{params:e}=r.detail;f.runDebugEncounter(e)});m.addEventListener("enter-workshop",()=>{f.enterWorkshop()});m.addEventListener("purchase-item",r=>{const{itemId:e}=r.detail;f.purchaseItem(e)});m.addEventListener("start-run",()=>{f.startNewRun()});m.addEventListener("start-game",()=>{f.startNewGame()});async function me(){m.innerHTML=`<div>${n("global.initializing")}</div>`,await P(),f.on("state-change",r=>{if(f.isLoading){m.innerHTML=`<div>${n("global.loading_game_data")}</div>`;return}if(f.error){m.innerHTML=`
