(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))t(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function s(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function t(n){if(n.ep)return;n.ep=!0;const a=s(n);fetch(n.href,a)}})();const O=15,M=3,N={Common:1,Uncommon:2,Rare:3},S={hp:100,maxHp:100,power:5},A=10,P=10,R=["loot_1","loot_2","loot_3","loot_4","loot_5"],q=32,x=9,D=300,C=r=>[...r].sort(()=>Math.random()-.5),H=(r,e)=>{let s=0;const t=new Map(e.map(l=>[l.id,l])),n=r.map(l=>t.get(l)).filter(Boolean),a=l=>(s++,{...l,instanceId:`${l.id}-${s}`}),i=n.filter(l=>l.cost===null),d=n.filter(l=>l.cost!==null);let u=[];if(i.length>0)for(const l of i)u.push(a(l),a(l),a(l),a(l));u.push(...d.map(a));let c=0;for(;u.length<q&&i.length>0;)u.push(a(i[c%i.length])),c++;return C(u)};class B{constructor(){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._listeners={},this.startNewRun=()=>{if(!this.gameState)return;const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:Math.min(100,this.gameState.adventurer.traits.expertise+5*(this.gameState.run-1))},s={...S,interest:100,traits:e,inventory:{weapon:null,armor:null,potions:[]}},t=H(this.gameState.unlockedDeck,this._allItems),n=t.slice(0,x),a=t.slice(x);this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",adventurer:s,availableDeck:a,hand:n,floor:1,feedback:"A new adventurer enters the dungeon!",log:[`--- Starting Run ${this.gameState.run} ---`],gameOver:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=e=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(t=>e.includes(t.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;const{choice:t,reason:n,logs:a}=this._getAdventurerChoice(this.gameState.adventurer,this.gameState.offeredLoot);let i={...this.gameState.adventurer},d=i.interest,u=this.gameState.hand,c=this.gameState.availableDeck;u.forEach(o=>o.justDrafted=!1);let l=u.filter(o=>!e.includes(o.instanceId));const p=x-l.length,b=c.slice(0,p);b.forEach(o=>{o.draftedFloor=this.gameState.floor,o.justDrafted=!0});const f=c.slice(p);if(l.push(...b),t){let o={...i.inventory,potions:[...i.inventory.potions]};t.type==="Weapon"?o.weapon=t:t.type==="Armor"?o.armor=t:t.type==="Potion"&&o.potions.push(t);const{power:h,maxHp:v}=this._recalculateStats(o),w=v-i.maxHp;i.inventory=o,i.power=h,i.maxHp=v,i.hp+=Math.max(0,w)}else d=Math.max(0,this.gameState.adventurer.interest-10);this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:{...i,interest:d},feedback:n,availableDeck:f,hand:l,log:[...this.gameState.log,...a]},this._emit("state-change",this.gameState)},D)},this.runDebugEncounter=e=>{!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_DIFFICULTY"||(this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.debugEncounterParams=e,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.debugEncounterParams)return;const{newAdventurer:s,feedback:t,logs:n}=this._getDebugEncounterOutcome(this.gameState.adventurer,this.gameState.floor,this.gameState.debugEncounterParams),a=this.gameState.floor+1,i=this.gameState.designer.balancePoints+P,d=[...this.gameState.log,...n];if(s.hp<=0){d.push("GAME OVER: Adventurer has fallen in battle."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:i},phase:"RUN_OVER",gameOver:{isOver:!0,reason:`The adventurer fell on floor ${this.gameState.floor}.`},log:d},this._emit("state-change",this.gameState);return}if(s.interest<=O){d.push("GAME OVER: Adventurer lost interest and left."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:i},phase:"RUN_OVER",gameOver:{isOver:!0,reason:`The adventurer grew bored on floor ${this.gameState.floor} and left.`},log:d},this._emit("state-change",this.gameState);return}let u="DESIGNER_CHOOSING_LOOT",c=t;this.gameState.hand&&this.gameState.hand.length===0&&(u="DESIGNER_CHOOSING_DIFFICULTY",d.push("Your hand is empty! The adventurer must press on without new items."),c="The adventurer waits for your decision, unaware that you have nothing left to offer."),this.gameState={...this.gameState,phase:u,adventurer:s,floor:a,designer:{balancePoints:i},feedback:c,log:d,debugEncounterParams:void 0},this._emit("state-change",this.gameState)},D))},this.enterWorkshop=()=>{if(!this.gameState)return;const e=this.gameState.run+1,s=this._allItems.filter(t=>t.cost!==null).filter(t=>!this.gameState.unlockedDeck.includes(t.id)).filter(t=>e>=t.minRun);this.gameState={...this.gameState,phase:"SHOP",run:e,floor:0,shopItems:C(s).slice(0,4),gameOver:{isOver:!1,reason:""},feedback:"Welcome back. Spend your Balance Points wisely."},this._emit("state-change",this.gameState)},this.purchaseItem=e=>{if(!this.gameState)return;const s=this._allItems.find(i=>i.id===e);if(!s||s.cost===null||this.gameState.designer.balancePoints<s.cost)return;const t=[...this.gameState.unlockedDeck,e],n=this.gameState.designer.balancePoints-s.cost,a=this.gameState.shopItems.filter(i=>i.id!==e);this.gameState={...this.gameState,designer:{balancePoints:n},unlockedDeck:t,shopItems:a,log:[...this.gameState.log,`Purchased ${s.name}.`]},this._emit("state-change",this.gameState)},this._loadGameData()}on(e,s){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(s)}_emit(e,s){const t=this._listeners[e];t&&t.forEach(n=>n(s))}_recalculateStats(e){let s=S.power,t=S.maxHp;return e.weapon&&(s+=e.weapon.stats.power||0,t+=e.weapon.stats.maxHp||0),e.armor&&(s+=e.armor.stats.power||0,t+=e.armor.stats.maxHp||0),{power:s,maxHp:t}}_getAdventurerChoice(e,s){var b,f;const{traits:t,inventory:n}=e,a=[`--- Adventurer Decision --- (Offense: ${t.offense}, Risk: ${t.risk})`],i=((b=n.weapon)==null?void 0:b.stats.power)||0,d=((f=n.armor)==null?void 0:f.stats.maxHp)||0;a.push(`Current Gear: Weapon Power(${i}), Armor HP(${d})`);const u=o=>{var v,w;let h=(N[o.rarity]||1)*5;switch(o.type){case"Weapon":const _=(o.stats.power||0)-i;if(_<=0&&o.id!==((v=n.weapon)==null?void 0:v.id))return-1;h+=_*(t.offense/10),_>0&&(h+=_*(t.expertise/10));const k=o.stats.maxHp||0;k<0&&(h+=k*(100-t.risk)/20);break;case"Armor":const y=(o.stats.maxHp||0)-d;if(y<=0&&o.id!==((w=n.armor)==null?void 0:w.id))return-1;h+=y*(100-t.offense)/10,y>0&&(h+=y*(t.expertise/10));const T=o.stats.power||0;T>0&&(h+=T*(t.offense/15));const L=o.stats.power||0;L<0&&(h+=L*(t.risk/10));break;case"Potion":const $=e.hp/e.maxHp;h+=10*(100-t.risk)/100,$<.7&&(h+=20*(1-$)),h+=5*(t.expertise/100),n.potions.length>=M&&(h*=.1);break}return h+Math.random()},c=s.map(o=>({item:o,score:u(o)})).filter(o=>o.score>0);if(c.sort((o,h)=>h.score-o.score),c.length===0||c[0].score<A)return{choice:null,reason:"They examine your offers but decide to take nothing.",logs:a};const l=c[0].item;let p=`They pick the ${l.name}.`;return{choice:l,reason:p,logs:a}}_getDebugEncounterOutcome(e,s,t){const n=[`--- DEBUG Encounter: Floor ${s} ---`];let a=JSON.parse(JSON.stringify(e)),i="";const d=a.hp/a.maxHp,u=1-a.traits.risk/120;if(d<u&&a.inventory.potions.length>0){const o=a.inventory.potions.shift(),h=o.stats.hp||0;a.hp=Math.min(a.maxHp,a.hp+h),i=`Sensing danger, they drink a ${o.name}. `}let c=Math.round(t.baseDamage*t.difficultyFactor);c=Math.max(0,c),a.hp-=c;let l=0,p="";const b=c/a.maxHp;let f="Normal";return t.difficultyFactor<=.8?f="Easy":t.difficultyFactor>=1.3&&(f="Hard"),f==="Easy"?(l=-5-Math.floor((100-a.traits.risk)/20),p="An easy fight. Too easy..."):b<.1?(l=-2,p="A worthy, but simple challenge."):b<.35?(l=5+Math.floor(a.traits.offense/20)+(f==="Hard"?5:0),p="A great battle! They feel alive!"):(l=-10-Math.floor((100-a.traits.risk)/10),p="That was far too close for comfort."),a.interest=Math.max(0,Math.min(100,a.interest+l)),{newAdventurer:a,feedback:i+p,damageTaken:c,logs:n}}async _loadGameData(){try{const e=await fetch("/game/items.json");if(!e.ok)throw new Error(`Failed to load items.json: ${e.statusText}`);this._allItems=await e.json();const s=1,t=R,n={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:0},a={...S,interest:100,traits:n,inventory:{weapon:null,armor:null,potions:[]}},i=H(t,this._allItems),d=i.slice(0,x),u=i.slice(x);this.gameState={phase:"DESIGNER_CHOOSING_LOOT",designer:{balancePoints:0},adventurer:a,unlockedDeck:t,availableDeck:u,hand:d,shopItems:[],offeredLoot:[],feedback:"A new adventurer enters the dungeon!",log:[`--- Starting Run ${s} ---`],run:s,floor:1,gameOver:{isOver:!1,reason:""}}}catch(e){this.error=e.message||"An unknown error occurred while loading game data.",this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}class G extends HTMLElement{constructor(){super(),this._balancePoints=0,this._run=0,this._floor=0}static get observedAttributes(){return["balance-points","run","floor"]}attributeChangedCallback(e,s,t){switch(e){case"balance-points":this._balancePoints=Number(t);break;case"run":this._run=Number(t);break;case"floor":this._floor=Number(t);break}this.render()}connectedCallback(){this.render()}render(){this.innerHTML=`
