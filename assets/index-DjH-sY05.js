(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))t(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function s(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function t(r){if(r.ep)return;r.ep=!0;const a=s(r);fetch(r.href,a)}})();const C=15,M=3,O={Common:1,Uncommon:2,Rare:3},_={hp:100,maxHp:100,power:5},N=10,A=10,P=["loot_1","loot_2","loot_3","loot_4","loot_5"],q=32,S=9,D=300,H=n=>[...n].sort(()=>Math.random()-.5),R=(n,e)=>{let s=0;const t=new Map(e.map(d=>[d.id,d])),r=n.map(d=>t.get(d)).filter(Boolean),a=d=>(s++,{...d,instanceId:`${d.id}-${s}`}),i=r.filter(d=>d.cost===null),c=r.filter(d=>d.cost!==null);let u=[];if(i.length>0)for(const d of i)u.push(a(d),a(d),a(d),a(d));u.push(...c.map(a));let l=0;for(;u.length<q&&i.length>0;)u.push(a(i[l%i.length])),l++;return H(u)};class B{constructor(){this.gameState=null,this.isLoading=!0,this.error=null,this._allItems=[],this._listeners={},this.startNewRun=()=>{if(!this.gameState)return;const e={offense:Math.floor(Math.random()*81)+10,risk:Math.floor(Math.random()*81)+10,expertise:Math.min(100,this.gameState.adventurer.traits.expertise+5*(this.gameState.run-1))},s={..._,interest:33+Math.floor(Math.random()*50),traits:e,inventory:{weapon:null,armor:null,potions:[]}},t=R(this.gameState.unlockedDeck,this._allItems),r=t.slice(0,S),a=t.slice(S);this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_LOOT",adventurer:s,availableDeck:a,hand:r,floor:1,feedback:"A new adventurer enters the dungeon!",log:[`--- Starting Run ${this.gameState.run} ---`],gameOver:{isOver:!1,reason:""}},this._emit("state-change",this.gameState)},this.presentOffer=e=>{if(!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_LOOT"||!this.gameState.hand)return;const s=this.gameState.hand.filter(t=>e.includes(t.instanceId));this.gameState.phase="AWAITING_ADVENTURER_CHOICE",this.gameState.offeredLoot=s,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ADVENTURER_CHOICE"||!this.gameState.hand)return;const{choice:t,reason:r,logs:a}=this._getAdventurerChoice(this.gameState.adventurer,this.gameState.offeredLoot);let i={...this.gameState.adventurer},c=i.interest,u=this.gameState.hand,l=this.gameState.availableDeck;u.forEach(o=>o.justDrafted=!1);let d=u.filter(o=>!e.includes(o.instanceId));const p=S-d.length,b=l.slice(0,p);b.forEach(o=>{o.draftedFloor=this.gameState.floor,o.justDrafted=!0});const f=l.slice(p);if(d.push(...b),t){let o={...i.inventory,potions:[...i.inventory.potions]};t.type==="Weapon"?o.weapon=t:t.type==="Armor"?o.armor=t:t.type==="Potion"&&o.potions.push(t);const{power:h,maxHp:v}=this._recalculateStats(o),x=v-i.maxHp;i.inventory=o,i.power=h,i.maxHp=v,i.hp+=Math.max(0,x)}else c=Math.max(0,this.gameState.adventurer.interest-10);this.gameState={...this.gameState,phase:"DESIGNER_CHOOSING_DIFFICULTY",adventurer:{...i,interest:c},feedback:r,availableDeck:f,hand:d,log:[...this.gameState.log,...a]},this._emit("state-change",this.gameState)},D)},this.runDebugEncounter=e=>{!this.gameState||this.gameState.phase!=="DESIGNER_CHOOSING_DIFFICULTY"||(this.gameState.phase="AWAITING_ENCOUNTER_FEEDBACK",this.gameState.debugEncounterParams=e,this._emit("state-change",this.gameState),setTimeout(()=>{if(!this.gameState||this.gameState.phase!=="AWAITING_ENCOUNTER_FEEDBACK"||!this.gameState.debugEncounterParams)return;const{newAdventurer:s,feedback:t,logs:r}=this._getDebugEncounterOutcome(this.gameState.adventurer,this.gameState.floor,this.gameState.debugEncounterParams),a=this.gameState.floor+1,i=this.gameState.designer.balancePoints+A,c=[...this.gameState.log,...r];if(s.hp<=0){c.push("GAME OVER: Adventurer has fallen in battle."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:i},phase:"RUN_OVER",gameOver:{isOver:!0,reason:`The adventurer fell on floor ${this.gameState.floor}.`},log:c},this._emit("state-change",this.gameState);return}if(s.interest<=C){c.push("GAME OVER: Adventurer lost interest and left."),this.gameState={...this.gameState,adventurer:s,designer:{balancePoints:i},phase:"RUN_OVER",gameOver:{isOver:!0,reason:`The adventurer grew bored on floor ${this.gameState.floor} and left.`},log:c},this._emit("state-change",this.gameState);return}let u="DESIGNER_CHOOSING_LOOT",l=t;this.gameState.hand&&this.gameState.hand.length===0&&(u="DESIGNER_CHOOSING_DIFFICULTY",c.push("Your hand is empty! The adventurer must press on without new items."),l="The adventurer waits for your decision, unaware that you have nothing left to offer."),this.gameState={...this.gameState,phase:u,adventurer:s,floor:a,designer:{balancePoints:i},feedback:l,log:c,debugEncounterParams:void 0},this._emit("state-change",this.gameState)},D))},this.enterWorkshop=()=>{if(!this.gameState)return;const e=this.gameState.run+1,s=this._allItems.filter(t=>t.cost!==null).filter(t=>!this.gameState.unlockedDeck.includes(t.id)).filter(t=>e>=t.minRun);this.gameState={...this.gameState,phase:"SHOP",run:e,floor:0,shopItems:H(s).slice(0,4),gameOver:{isOver:!1,reason:""},feedback:"Welcome back. Spend your Balance Points wisely."},this._emit("state-change",this.gameState)},this.purchaseItem=e=>{if(!this.gameState)return;const s=this._allItems.find(i=>i.id===e);if(!s||s.cost===null||this.gameState.designer.balancePoints<s.cost)return;const t=[...this.gameState.unlockedDeck,e],r=this.gameState.designer.balancePoints-s.cost,a=this.gameState.shopItems.filter(i=>i.id!==e);this.gameState={...this.gameState,designer:{balancePoints:r},unlockedDeck:t,shopItems:a,log:[...this.gameState.log,`Purchased ${s.name}.`]},this._emit("state-change",this.gameState)},this._loadGameData()}on(e,s){this._listeners[e]||(this._listeners[e]=[]),this._listeners[e].push(s)}_emit(e,s){const t=this._listeners[e];t&&t.forEach(r=>r(s))}_recalculateStats(e){let s=_.power,t=_.maxHp;return e.weapon&&(s+=e.weapon.stats.power||0,t+=e.weapon.stats.maxHp||0),e.armor&&(s+=e.armor.stats.power||0,t+=e.armor.stats.maxHp||0),{power:s,maxHp:t}}_getAdventurerChoice(e,s){var b,f;const{traits:t,inventory:r}=e,a=[`--- Adventurer Decision --- (Offense: ${t.offense}, Risk: ${t.risk})`],i=((b=r.weapon)==null?void 0:b.stats.power)||0,c=((f=r.armor)==null?void 0:f.stats.maxHp)||0;a.push(`Current Gear: Weapon Power(${i}), Armor HP(${c})`);const u=o=>{var v,x;let h=(O[o.rarity]||1)*5;switch(o.type){case"Weapon":const w=(o.stats.power||0)-i;if(w<=0&&o.id!==((v=r.weapon)==null?void 0:v.id))return-1;h+=w*(t.offense/10),w>0&&(h+=w*(t.expertise/10));const T=o.stats.maxHp||0;T<0&&(h+=T*(100-t.risk)/20);break;case"Armor":const y=(o.stats.maxHp||0)-c;if(y<=0&&o.id!==((x=r.armor)==null?void 0:x.id))return-1;h+=y*(100-t.offense)/10,y>0&&(h+=y*(t.expertise/10));const k=o.stats.power||0;k>0&&(h+=k*(t.offense/15));const L=o.stats.power||0;L<0&&(h+=L*(t.risk/10));break;case"Potion":const $=e.hp/e.maxHp;h+=10*(100-t.risk)/100,$<.7&&(h+=20*(1-$)),h+=5*(t.expertise/100),r.potions.length>=M&&(h*=.1);break}return h+Math.random()},l=s.map(o=>({item:o,score:u(o)})).filter(o=>o.score>0);if(l.sort((o,h)=>h.score-o.score),l.length===0||l[0].score<N)return{choice:null,reason:"They examine your offers but decide to take nothing.",logs:a};const d=l[0].item;let p=`They pick the ${d.name}.`;return{choice:d,reason:p,logs:a}}_getDebugEncounterOutcome(e,s,t){const r=[`--- DEBUG Encounter: Floor ${s} ---`];let a=JSON.parse(JSON.stringify(e)),i="";const c=a.hp/a.maxHp,u=1-a.traits.risk/120;if(c<u&&a.inventory.potions.length>0){const o=a.inventory.potions.shift(),h=o.stats.hp||0;a.hp=Math.min(a.maxHp,a.hp+h),i=`Sensing danger, they drink a ${o.name}. `}let l=Math.round(t.baseDamage*t.difficultyFactor);l=Math.max(0,l),a.hp-=l;let d=0,p="";const b=l/a.maxHp;let f="Normal";return t.difficultyFactor<=.8?f="Easy":t.difficultyFactor>=1.3&&(f="Hard"),f==="Easy"?(d=-5-Math.floor((100-a.traits.risk)/20),p="An easy fight. Too easy..."):b<.1?(d=-2,p="A worthy, but simple challenge."):b<.35?(d=5+Math.floor(a.traits.offense/20)+(f==="Hard"?5:0),p="A great battle! They feel alive!"):(d=-10-Math.floor((100-a.traits.risk)/10),p="That was far too close for comfort."),a.interest=Math.max(0,Math.min(100,a.interest+d)),{newAdventurer:a,feedback:i+p,damageTaken:l,logs:r}}async _loadGameData(){try{const e=await fetch("/game/items.json");if(!e.ok)throw new Error(`Failed to load items.json: ${e.statusText}`);this._allItems=await e.json(),this.gameState={phase:"LOADING",designer:{balancePoints:0},adventurer:{..._,interest:0,traits:{offense:0,risk:0,expertise:0},inventory:{weapon:null,armor:null,potions:[]}},unlockedDeck:P,availableDeck:[],hand:[],shopItems:[],offeredLoot:[],feedback:"",log:[],run:1,floor:0,gameOver:{isOver:!1,reason:""}},this.startNewRun()}catch(e){this.error=e.message||"An unknown error occurred while loading game data.",this._emit("error",null)}finally{this.isLoading=!1,this._emit("state-change",this.gameState)}}}class G extends HTMLElement{constructor(){super(),this._balancePoints=0,this._run=0,this._floor=0}static get observedAttributes(){return["balance-points","run","floor"]}attributeChangedCallback(e,s,t){switch(e){case"balance-points":this._balancePoints=Number(t);break;case"run":this._run=Number(t);break;case"floor":this._floor=Number(t);break}this.render()}connectedCallback(){this.render()}render(){this.innerHTML=`
        ...
        `}}customElements.define("adventurer-status",K);const Y={Common:"text-rarity-common",Uncommon:"text-rarity-uncommon",Rare:"text-rarity-rare"},E=(n,e)=>{const s=e>0;return`
    ...
    `};class z extends HTMLElement{constructor(){super(),this._item=null,this._isSelected=!1,this._isDisabled=!1,this._isNewlyDrafted=!1,this.addEventListener("click",()=>{!this._isDisabled&&this._item&&this.dispatchEvent(new CustomEvent("loot-card-select",{bubbles:!0,composed:!0,detail:{instanceId:this._item.instanceId}}))}),this.addEventListener("animationend",e=>{e.animationName==="newly-drafted-animation"&&this.classList.remove("animate-newly-drafted")})}set item(e){this._item=e,this.render()}get item(){return this._item}set isSelected(e){this._isSelected=e,this.render()}get isSelected(){return this._isSelected}set isDisabled(e){this._isDisabled=e,this.render()}get isDisabled(){return this._isDisabled}set isNewlyDrafted(e){this._isNewlyDrafted!==e&&(this._isNewlyDrafted=e,this._isNewlyDrafted&&this.classList.add("animate-newly-drafted"))}get isNewlyDrafted(){return this._isNewlyDrafted}connectedCallback(){this.render()}render(){if(!this._item)return;const e=Y[this._item.rarity]||"text-gray-400",s="bg-brand-surface border rounded-lg p-4 flex flex-col justify-between transition-all duration-200 shadow-lg";let t="";this._isDisabled?t="border-gray-600 opacity-50 cursor-not-allowed":this._isSelected?t="border-brand-secondary scale-105 ring-2 ring-brand-secondary cursor-pointer transform":t="border-brand-primary hover:border-brand-secondary cursor-pointer transform hover:scale-105";const r=this.classList.contains("animate-newly-drafted")?" animate-newly-drafted":"";this.className=`${s} ${t}${r}`,this.innerHTML=`
        ...
        `}updateDecision(e){const s=this.querySelector("#decision-container"),t=this.querySelector("#button-container");if(!s||!t)return;let r="",a="";const i=e?"animate-fade-in-up":"";this.decision==="continue"?(r=`
            ...
            `),s.innerHTML=r,t.innerHTML=a}}customElements.define("game-over-screen",se);const I=(n,e)=>{const s=e>0;return`
        <div class="flex justify-between text-sm ${s?"text-green-400":"text-red-400"}">
            <span>${n}</span>
            <span class="font-mono">${s?"+":""}${e}</span>
        </div>
        ...
        <div class="bg-brand-surface border border-brand-primary rounded-lg p-4 flex flex-col justify-between shadow-lg animate-fade-in">
            <div>
                ...
                <div class="space-y-1 text-brand-text mb-4">
                    ${n.stats.hp?I("Health",n.stats.hp):""}
                    ${n.stats.maxHp?I("Max HP",n.stats.maxHp):""}
                    ${n.stats.power?I("Power",n.stats.power):""}
                </div>
            </div>
    ...
    `;const s=document.querySelector("adventurer-status");s&&(s.adventurer=n.adventurer);const t=document.querySelector("loot-choice-panel");t&&(t.choices=n.hand,t.disabled=!1);const r=document.querySelector("debug-encounter-panel");if(r){const i=Math.max(1,15-Math.floor(n.adventurer.power/4)+Math.floor(n.floor*1.5));r.defaultBaseDamage=i}const a=document.querySelector("debug-log");a&&(a.logs=n.log,a.traits=n.adventurer.traits)};m.addEventListener("present-offer",n=>{const{ids:e}=n.detail;g.presentOffer(e)});m.addEventListener("run-encounter",n=>{const{params:e}=n.detail;g.runDebugEncounter(e)});m.addEventListener("enter-workshop",()=>{g.enterWorkshop()});m.addEventListener("purchase-item",n=>{const{itemId:e}=n.detail;g.purchaseItem(e)});m.addEventListener("start-run",()=>{g.startNewRun()});g.on("state-change",n=>{if(g.isLoading){m.innerHTML="<div>Loading Game Data...</div>";return}if(g.error){m.innerHTML=`
        ...
        `;return}oe(n)});m.innerHTML="<div>Initializing...</div>";
